...
test.describe('ACOM MAS cards feature test suite', () => {
  ...
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-2: Verify Plans Merch Card content', async () => {
      ...
      await expect(await acomPage.getCardDescription(data.id)).toBeVisible();
      await expect(await acomPage.getCardDescription(data.id)).toContainText(data.description);
