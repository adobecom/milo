...
test.describe('MAS Docs feature test suite', () => {
  ...
  test(`${features[1].name},${features[1].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-3: Verify refresh btn', async () => {
      const btnRefresh = page.locator('#btnRefreshFragments');
      const masFailedLog = page.locator('#log-mas-failed');
      await btnRefresh.click();
      await expect(masFailedLog).toContainText(/'mas:failed' on A #wrongosi: Error: Bad WCS request: 404, url:(.*), start: \d{13}, end: \d{13}'mas:failed' on A #wrongosi: Error: Bad WCS request: 404, url:(.*), start: \d{13}, end: \d{13}/);
    });
  });
});
