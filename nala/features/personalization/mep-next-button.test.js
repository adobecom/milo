...
// force: true bypasses Playwright's visibility/stability checks (needed because content
// keeps trickling in under CI's throttled EDS pacing), but it does not bypass the
// requirement that the click point be on-screen. With all cards defaulting to expanded
// the drawer is taller, so scroll explicitly first to avoid "outside of the viewport" errors.
...
test(`[Test Id - ${features[0].tcid}] ${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
  ...
  // see forceClick above for why we bypass actionability checks here
  await forceClick(mepButtonLoc.card2toggle1);
  await forceClick(mepButtonLoc.card2toggle2);
  await forceClick(mepButtonLoc.card2toggle3);
  await forceClick(mepButtonLoc.card2toggle4);
...
test(`[Test Id - ${features[2].tcid}] ${features[2].name},${features[2].tags}`, async ({ page, baseURL }) => {
  const URL = `${baseURL.replace('.aem.live', '.aem.page')}${features[2].path}${miloLibs}`;
  console.info(`[Test Page]: ${URL}`);
  await page.goto(URL);
...
test(`[Test Id - ${features[3].tcid}] ${features[3].name},${features[3].tags}`, async ({ page, baseURL }) => {
  const URL = `${baseURL.replace('.aem.live', '.aem.page')}${features[3].path}${miloLibs}`;
  console.info(`[Test Page]: ${URL}`);
  await page.goto(URL);
...
test(`[Test Id - ${features[4].tcid}] ${features[4].name},${features[4].tags}`, async ({ page, baseURL }) => {
  const URL = `${baseURL.replace('.aem.live', '.aem.page')}${features[4].path}${miloLibs}`;
  console.info(`[Test Page]: ${URL}`);
  await page.goto(URL);
  ...
  await forceClick(mepButtonLoc.toggleMepTrack);
...
test(`[Test Id - ${features[5].tcid}] ${features[5].name},${features[5].tags}`, async ({ page, baseURL }) => {
  const URL = `${baseURL.replace('.aem.live', '.aem.page')}${features[5].path}${miloLibs}`;
  console.info(`[Test Page]: ${URL}`);
  await page.goto(URL);
  ...
  await forceClick(mepButtonLoc.highlightExpandIcon);
...
test(`[Test Id - ${features[6].tcid}] ${features[6].name},${features[6].tags}`, async ({ page, baseURL }) => {
  const URL = `${baseURL.replace('.aem.live', '.aem.page')}${features[6].path}${miloLibs}`;
  console.info(`[Test Page]: ${URL}`);
  await page.goto(URL);
...
test(`[Test Id - ${features[7].tcid}] ${features[7].name},${features[7].tags}`, async ({ page, baseURL }) => {
  const URL = `${baseURL.replace('.aem.live', '.aem.page')}${features[7].path}${miloLibs}`;
  console.info(`[Test Page]: ${URL}`);
  await page.goto(URL);
  ...
  await forceClick(mepButtonLoc.previewLinkTrack);
...
test(`[Test Id - ${features[8].tcid}] ${features[8].name},${features[8].tags}`, async ({ page, baseURL }) => {
  const URL = `${baseURL.replace('.aem.live', '.aem.page')}${features[8].path}${miloLibs}`;
  console.info(`[Test Page]: ${URL}`);
  await page.goto(URL);
