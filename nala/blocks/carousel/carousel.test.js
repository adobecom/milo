  ...
  test.describe('Accessibility - WCAG 2.1 A/AA', () => {
    test(`${features[4].name},${features[4].tags}`, async ({ page, baseURL }) => {
      ...
      await test.step('A/AA - Tab Order Is Logical', async () => {
        ...
        const prevButtonFocused = await carousel.previousButton.evaluate((el) => el === document.activeElement);
        expect(prevButtonFocused).toBeTruthy();

        await page.keyboard.press('Tab');
        const nextButtonFocused = await carousel.nextButton.evaluate((el) => el === document.activeElement);
