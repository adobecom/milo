...
describe('Three-in-One XSS Sanitization', () => {
  describe('decodeUrl', () => {
    ...
    it('should handle malformed percent encoding gracefully', () => {
      ...
      expect(decodeUrl('test%ZZ')).to.equal('test%ZZ');
    });
    ...
    it('should stop when URL no longer changes between iterations', () => {
      const url = 'https://example.com/already%20decoded';
      const expected = 'https://example.com/already decoded';
      expect(decodeUrl(url)).to.equal(expected);
      expect(decodeUrl('test%21%40%23')).to.equal('test!@#');
    });

    it('should handle complex real-world scenarios', () => {
      expect(decodeUrl('https://example.com?redirect%3Dhttps%253A//other.com'))
        .to.equal('https://example.com?redirect=https://other.com');
      expect(decodeUrl('https://example.com%252Fpath%252Fwith%2520spaces'))
        .to.equal('https://example.com/path/with spaces');
      expect(decodeUrl('https://example.com%23fragment%2520test'))
        .to.equal('https://example.com#fragment test');
    });
  ...
  describe('normalizeUrl', () => {
    it('should return normal URLs unchanged', () => {
      ...
      expect(normalizeUrl('ftp://example.com')).to.equal('ftp://example.com');
    });

    it('should trim whitespace from URLs', () => {
      expect(normalizeUrl('  https://example.com  ')).to.equal('https://example.com');
      expect(normalizeUrl('\t\nhttps://example.com\r\n')).to.equal('https://example.com');
      expect(normalizeUrl('   /path/to/resource   ')).to.equal('/path/to/resource');
    });
    ...
    it('should remove ASCII control characters', () => {
      ...
      expect(normalizeUrl('https://example\x1f.com')).to.equal('https://example.com');
      expect(normalizeUrl('https://example.com\x7f')).to.equal('https://example.com');
      expect(normalizeUrl('https://\x0aexample\x0d.com')).to.equal('https://example.com');
    });
    ...
    it('should handle edge cases with whitespace and protocols', () => {
      // eslint-disable-next-line no-script-url
      expect(normalizeUrl('  javascript:alert(1)  ')).to.be.null;
      expect(normalizeUrl('  data:text/html,test  ')).to.be.null;
      expect(normalizeUrl('  https://example.com  ')).to.equal('https://example.com');
    });
  });
  ...
  describe('isHttpsAdobeDomain', () => {
    it('should return valid HTTPS Adobe.com URLs', () => {
      ...
      expect(isHttpsAdobeDomain('https://adobe.com/')).to.equal('https://adobe.com/');
      expect(isHttpsAdobeDomain('https://adobe.com/path')).to.equal('https://adobe.com/path');
    ...
    it('should return valid HTTPS Adobe subdomain URLs', () => {
      expect(isHttpsAdobeDomain('https://www.adobe.com')).to.equal('https://www.adobe.com/');
      ...
      expect(isHttpsAdobeDomain('https://my-subdomain.adobe.com/path')).to.equal('https://my-subdomain.adobe.com/path');
    ...
    it('should return null for HTTP (non-HTTPS) Adobe URLs', () => {
      ...
      expect(isHttpsAdobeDomain('http://www.adobe.com')).to.be.null;
    ...
    it('should return null for non-Adobe domains', () => {
      ...
      expect(isHttpsAdobeDomain('https://google.com')).to.be.null;
      expect(isHttpsAdobeDomain('https://adobe-fake.com')).to.be.null;
      expect(isHttpsAdobeDomain('https://fakeadobe.com')).to.be.null;
    ...
    it('should return null for domains that contain adobe.com but are not subdomains', () => {
      ...
      expect(isHttpsAdobeDomain('https://prefixadobe.com')).to.be.null;
    ...
    it('should return relative URLs unchanged', () => {
      ...
      expect(isHttpsAdobeDomain('/')).to.equal('/');
    ...
    it('should return null for malformed URLs', () => {
      ...
      expect(isHttpsAdobeDomain('https:///')).to.be.null;
      expect(isHttpsAdobeDomain('https://adobe.com:99999999')).to.be.null;
      expect(isHttpsAdobeDomain('https://[invalid-ipv6]')).to.be.null;
    });
    ...
    it('should handle URLs with ports', () => {
      expect(isHttpsAdobeDomain('https://adobe.com:443')).to.equal('https://adobe.com/');
      expect(isHttpsAdobeDomain('https://commerce.adobe.com:8080')).to.equal('https://commerce.adobe.com:8080/');
      expect(isHttpsAdobeDomain('http://adobe.com:80')).to.be.null; // HTTP not allowed
    });

    it('should handle URLs with authentication', () => {
      expect(isHttpsAdobeDomain('https://user:pass@adobe.com')).to.equal('https://user:pass@adobe.com/');
      expect(isHttpsAdobeDomain('https://user@commerce.adobe.com')).to.equal('https://user@commerce.adobe.com/');
    });
    ...
    it('should validate exact domain matching', () => {
      ...
      expect(isHttpsAdobeDomain('https://adobe.comm')).to.be.null;
      expect(isHttpsAdobeDomain('https://xadobe.com')).to.be.null;
    });
  });
  ...
  describe('sanitizeUrl', () => {
    ...
    it('should reject URLs that fail at the domain validation step', () => {
      ...
      expect(sanitizeUrl('https://google.com')).to.be.null;
    ...
    it('should properly trim input before processing', () => {
      expect(sanitizeUrl('   https://adobe.com   ')).to.equal('https://adobe.com/');
      expect(sanitizeUrl('\t\nhttps://commerce.adobe.com\r\n')).to.equal('https://commerce.adobe.com/');
    });

    it('should handle integration of all security measures', () => {
      expect(sanitizeUrl('  https%25253A//commerce\x00.adobe.com%252Fpath  ')).to.equal('https://commerce.adobe.com/path');
      // eslint-disable-next-line no-script-url
      expect(sanitizeUrl('  javascript%25253Aalert%25281%2529  ')).to.be.null;
      expect(sanitizeUrl('  https%25253A//evil\x00.com%252Fpath  ')).to.be.null;
    });
  });
  ...
  describe('sanitizeTarget', () => {
    ...
    it('should return _blank for non-string values', () => {
    ...
    it('should handle whitespace around valid targets', () => {
      expect(sanitizeTarget('  _blank  ')).to.equal('_blank');
      expect(sanitizeTarget('\t_self\n')).to.equal('_self');
      expect(sanitizeTarget(' _parent ')).to.equal('_parent');
      expect(sanitizeTarget('_top\r')).to.equal('_top');
    });

    it('should return valid custom target names', () => {
      expect(sanitizeTarget('myWindow')).to.equal('myWindow');
      expect(sanitizeTarget('window123')).to.equal('window123');
      expect(sanitizeTarget('my-window')).to.equal('my-window');
      expect(sanitizeTarget('my_window')).to.equal('my_window');
      expect(sanitizeTarget('Window-123_test')).to.equal('Window-123_test');
    });

    it('should sanitize control characters and return valid result', () => {
      // Control characters that get stripped but result in valid target
      expect(sanitizeTarget('_\x00blank')).to.equal('_blank');
      expect(sanitizeTarget('_\x1fself')).to.equal('_self');
      expect(sanitizeTarget('_\u0000parent')).to.equal('_parent');
      expect(sanitizeTarget('_\u2000top')).to.equal('_top');
    });
    ...
    it('should return _blank for empty string after sanitization', () => {
      expect(sanitizeTarget('\x00\x1f\x7f')).to.equal('_blank');
      expect(sanitizeTarget('\u0000\u001F\u007F\u009F')).to.equal('_blank');
      expect(sanitizeTarget('   \t\n\r   ')).to.equal('_blank');
    });
  });
});
