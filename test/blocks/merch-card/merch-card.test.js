...
describe('Simplified Pricing Express Card', () => {
  it('Supports Simplified Pricing Express card', async () => {
    document.body.innerHTML = await readMockText('/test/blocks/merch-card/mocks/simplified-pricing-express.html');
    const merchCard = await init(document.querySelector('.simplified-pricing-express'));
    expectToValidateHTMLAssertions(merchCard, {
      elements: [
        { selector: 'h2[slot="heading-l"]', textContent: 'Express - Create standout content' },
        { selector: 'p[slot="body-s"]', textContent: 'Make amazing social content, flyers, and more. Stand out with Adobe Express.' },
        { attribute: { name: 'variant', value: 'simplified-pricing-express' } },
        { attribute: { name: 'badge-background-color', value: '#FFD700' } },
        { attribute: { name: 'badge-color', value: '#000000' } },
        { attribute: { name: 'badge-text', value: 'LIMITED TIME OFFER' } },
      ],
  ...
  it('Supports Simplified Pricing Express card with price', async () => {
    document.body.innerHTML = await readMockText('/test/blocks/merch-card/mocks/simplified-pricing-express.html');
    const merchCard = await init(document.querySelector('.simplified-pricing-express.green'));
    expectToValidateHTMLAssertions(merchCard, {
      elements: [
        { selector: 'h2[slot="heading-l"]', textContent: 'Express Premium' },
        { selector: 'p[slot="body-s"]', textContent: 'Unlock premium templates, fonts, and features. Create without limits.' },
        { selector: 'span[slot="price"]' },
        { attribute: { name: 'variant', value: 'simplified-pricing-express' } },
        { attribute: { name: 'badge-background-color', value: '#00C853' } },
        { attribute: { name: 'badge-color', value: '#000000' } },
        { attribute: { name: 'badge-text', value: 'BEST VALUE' } },
      ],
      buttons: ['Get Premium'],
    });
  });
  ...
  it('Supports Simplified Pricing Express card with custom border color', async () => {
    document.body.innerHTML = await readMockText('/test/blocks/merch-card/mocks/simplified-pricing-express.html');
    const merchCard = await init(document.querySelector('[data-color="#E91E63"]'));
    expectToValidateHTMLAssertions(merchCard, {
      elements: [
        { selector: 'h2[slot="heading-l"]', textContent: 'Express with AI' },
        { selector: 'p[slot="body-s"]', textContent: 'Create content faster with AI-powered tools and generative features.' },
        { selector: 'span[slot="price"]' },
        { attribute: { name: 'variant', value: 'simplified-pricing-express' } },
        { attribute: { name: 'badge-text', value: 'NEW' } },
        { attribute: { name: 'border-color', value: '#E91E63' } },
      ],
      buttons: ['Try AI features'],
    });
  });

  it('Supports Simplified Pricing Express card with multiple CTAs', async () => {
    document.body.innerHTML = await readMockText('/test/blocks/merch-card/mocks/simplified-pricing-express.html');
    const merchCard = await init(document.querySelector('.simplified-pricing-express.blue'));
    expectToValidateHTMLAssertions(merchCard, {
      elements: [
        { selector: 'h2[slot="heading-l"]', textContent: 'Express for teams' },
        { selector: 'p[slot="body-s"]', textContent: 'Collaborate on content creation with advanced sharing and brand controls.' },
        { selector: 'span[slot="price"]' },
        { attribute: { name: 'variant', value: 'simplified-pricing-express' } },
        { attribute: { name: 'badge-background-color', value: '#1976D2' } },
        { attribute: { name: 'badge-color', value: '#FFFFFF' } },
        { attribute: { name: 'badge-text', value: 'EXCLUSIVE' } },
      ],
