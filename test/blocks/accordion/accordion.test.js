...
describe('Accordion', () => {
  ...
  it('Runs all basic functions of the accordion', async () => {
    ...
    expect(firstAccordionButton.getAttribute('daa-ll')).to.equal(`open-1|${'What if my dough did'.trim().slice(0, 30)}`);
    firstAccordionButton.click();
    expect(firstAccordionButton.getAttribute('aria-expanded')).to.equal('true');
    expect(firstAccordionButton.getAttribute('daa-ll')).to.equal(`close-1|${'What if my dough did'.trim().slice(0, 30)}`);
