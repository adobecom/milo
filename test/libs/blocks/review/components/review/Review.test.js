...
describe('Review', () => {
  ...
  it('should display review', async () => {
    const reviewElement = await waitForElement(
      '.hlx-ReviewWrapper'
    );
  ...
  it ('should test ratings click above comment threshold', async () => {
    ...
    const ratingsElement = reviewElement.querySelectorAll('.hlx-Review-ratingFields input')[4];
  ...
  it ('should test ratings click above comment threshold', async () => {
    ...
    const ratingsElement = reviewElement.querySelectorAll('.hlx-Review-ratingFields input')[1];
    ratingsElement.dispatchEvent(new Event('click'));
    expect(ratingsElement.classList.contains('is-active')).to.be.false;
  });
  ...
  it ('should test for input change', async () => {
    ...
    const commentElem = reviewElement.querySelector('.hlx-Review-commentFields');
