...
describe('Utils', () => {
  ...
  describe('externalLibs configuration', () => {
    let originalConfig;
    let originalImport;
    let importPaths;
    ...
    describe('loadBlock with externalLibs', () => {
      it('should attempt to load block from external library (single object config)', async () => {
        ...
        mockBlock.classList.add('custom-block');
        mockBlock.setAttribute('data-test-block', 'true');
        document.body.appendChild(mockBlock);

        await utils.loadBlock(mockBlock);

        expect(importPaths).to.include('https://external.example.com/blocks/custom-block/custom-block.js');
      });
      ...
      it('should attempt to load block from external library (array config)', async () => {
        ...
        expect(importPaths).to.include('https://lib2.example.com/blocks/custom-block/custom-block.js');
      });
      ...
      it('should use first matching external library when multiple matches exist', async () => {
        ...
        const mockBlock = document.createElement('div');
        mockBlock.classList.add('shared-block');
        mockBlock.setAttribute('data-test-block', 'true');
        document.body.appendChild(mockBlock);

        await utils.loadBlock(mockBlock);

        expect(importPaths).to.include('https://first.example.com/blocks/shared-block/shared-block.js');
        expect(importPaths).to.not.include('https://second.example.com/blocks/shared-block/shared-block.js');
      });
      ...
      it('should fall back to codeRoot when block not found in external libraries', async () => {
        ...
        const mockBlock = document.createElement('div');
        mockBlock.classList.add('regular-block');
        mockBlock.setAttribute('data-test-block', 'true');
        document.body.appendChild(mockBlock);

        await utils.loadBlock(mockBlock);

        expect(importPaths).to.include('/libs/blocks/regular-block/regular-block.js');
      });
      ...
      it('should prioritize miloLibs over externalLibs for MILO_BLOCKS', async () => {
        ...
        const mockBlock = document.createElement('div');
        mockBlock.classList.add('marquee');
        mockBlock.setAttribute('data-test-block', 'true');
        document.body.appendChild(mockBlock);

        await utils.loadBlock(mockBlock);

        expect(importPaths).to.include('https://milo.adobe.com/libs/blocks/marquee/marquee.js');
        expect(importPaths).to.not.include('https://external.example.com/blocks/marquee/marquee.js');
      });
      ...
      it('should prioritize externalLibs over codeRoot for non-MILO blocks', async () => {
        ...
        expect(importPaths).to.include('https://external.example.com/blocks/custom-block/custom-block.js');
        expect(importPaths).to.not.include('/libs/blocks/custom-block/custom-block.js');
      });
      ...
      it('should handle mixed valid and invalid external library configurations', async () => {
        ...
        // Reset import paths for next test
        importPaths = [];

        // Test invalid configuration falls back to codeRoot
        const invalidBlock = document.createElement('div');
        invalidBlock.classList.add('invalid-block');
        invalidBlock.setAttribute('data-test-block', 'true');
        document.body.appendChild(invalidBlock);

        await utils.loadBlock(invalidBlock);
        expect(importPaths).to.include('/libs/blocks/invalid-block/invalid-block.js');
      });
      ...
      it('should respect MEP block overrides even with externalLibs', async () => {
        ...
        expect(importPaths).to.include('https://mep.example.com/custom-path/test-block.js');
        expect(importPaths).to.not.include('https://external.example.com/blocks/test-block/test-block.js');
      });
      ...
      it('should handle externalLibs with non-string block names gracefully', async () => {
        ...
        const mockBlock = document.createElement('div');
        mockBlock.classList.add('valid-block');
        mockBlock.setAttribute('data-test-block', 'true');
        document.body.appendChild(mockBlock);
