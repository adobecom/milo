...
function preloadLanguageBanner() {
  const config = getConfig();
  ...
  // Preload JS module
  const { base } = config;
  loadLink(`${base}/utils/language-banner-utils.js`, {
    rel: 'modulepreload',
...
async function processSection(section, config, isDoc, lcpSectionId) {
  ...
  if (isLcpSection) {
    const { getLangBannerPromise } = await import('./language-banner-utils.js');
    const langBannerPromise = getLangBannerPromise();
    if (langBannerPromise) await langBannerPromise;
  }
