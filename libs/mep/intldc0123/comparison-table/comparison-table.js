...
import { getMetadata as getSectionMetadata } from '../../../blocks/section-metadata/section-metadata.js';
...
function getHeaderColumnOffset(el) {
  return el.querySelector('.header-item[data-column-index="0"]') ? 1 : 0;
}

function getCompactColumns(el) {
  const sectionMetadata = el.closest('.section')?.querySelector('.section-metadata');
  const text = sectionMetadata && getSectionMetadata(sectionMetadata)['compact columns']?.text?.trim();
  if (!text) return null;
  return text.split(',').map((n) => parseInt(n.trim(), 10)).filter((n) => n > 0);
}

function setupResponsiveHiding(el) {
  const compactLayoutQuery = window.matchMedia('(max-width: 899px)');
  const compactColumns = getCompactColumns(el);

  const setCompactVisibility = (isMobile) => {
    const headerItems = [...el.querySelectorAll('.header-item[data-column-index]')];
    if (headerItems.length <= 2) return;

    const headerOffset = getHeaderColumnOffset(el);
    const visibleColumns = compactColumns || [1, 2];
    const visibleHeader = new Set(visibleColumns.map((n) => String(n - headerOffset)));
    const visibleCell = new Set(visibleColumns.map(String));

    headerItems.forEach((item) => {
      const show = !isMobile || visibleHeader.has(item.getAttribute('data-column-index'));
      item.classList.toggle('hidden', !show);
    });

    el.querySelectorAll('.table-row').forEach((row) => {
      row.querySelectorAll('.table-cell').forEach((cell) => {
        const show = !isMobile || visibleCell.has(cell.getAttribute('data-column-index'));
...
function setupStickyHeader(el) {
  ...
  const calculateTotalOffset = () => {
    const header = document.querySelector('header.global-navigation, header');
    const localNavHeight = document.querySelector('.feds-localnav')?.offsetHeight || 0;
    if (!header) return localNavHeight;

    const { position } = getComputedStyle(header);
    const navBottom = (position === 'fixed' || position === 'sticky')
      ? Math.max(0, Math.round(header.getBoundingClientRect().bottom))
      : 0;
    return navBottom + localNavHeight;
  };
...
function handleColumnBackgroundColor(text, el) {
  if (!text?.trim()) return;
  const entries = getIndexedValues(text);
  const headerOffset = getHeaderColumnOffset(el);
  const planColumnCount = el.querySelectorAll('.header-item[data-column-index]').length;

  const paintColumn = (columnNumber, value) => {
    const headerItem = el.querySelector(`.header-item[data-column-index="${columnNumber - headerOffset}"]`);
    [...(headerItem?.querySelectorAll('.sub-header-item-container') || [])]
      .slice(0, 2)
      .forEach((node) => {
        node.style.setProperty('background', value, 'important');
        node.classList.add('has-custom-column-bg');
      });
    el.querySelectorAll(`.table-cell[data-column-index="${columnNumber}"] > div`).forEach((node) => {
      node.style.setProperty('background', value, 'important');
      node.classList.add('has-custom-column-bg');
    });
  };
  ...
  if (entries.length === 1 && entries[0].length === 1) {
    for (let columnNumber = 1; columnNumber <= planColumnCount; columnNumber += 1) {
      paintColumn(columnNumber, entries[0][0]);
    }
    return;
  }

  entries.forEach(([index, value]) => {
    const columnNumber = parseInt(index, 10);
    if (columnNumber > 0 && value) paintColumn(columnNumber, value);
  });
}

function applySectionMetadata(el) {
  const sectionMetadata = el.closest('.section')?.querySelector('.section-metadata');
  if (!sectionMetadata) return;
  const metadata = getSectionMetadata(sectionMetadata);
  const columnBackground = metadata['column background color']?.content?.textContent?.trim();
  if (columnBackground) handleColumnBackgroundColor(columnBackground, el);
}
