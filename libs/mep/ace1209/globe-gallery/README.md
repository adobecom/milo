...
| `--gg-pq-hold` | how much tail the held quote spends | **published by JS, not authored** (`publishPqMetrics`) — it is the entire gap between the quote's bottom edge and the next section's top, so the hold spends all of it. It does **not** change the reveal, which runs on its own clock |
...
while it was invisible), stays there for `--gg-pq-hold` of scroll, then is
...
- *Whole stretch after the globe too long, or too much blank before the quote:* shorten the tail. The
  ...
  - `--gg-pq-hold` is **not** a knob — it is derived, and by construction spends the whole gap.

  Floor on all three: `(1 − appear-t) × tail` must stay above half the quote box *plus the hold*, or the
  next section lands on the quote. Shrinking the tail also shrinks `--gg-pq-hold`, so the hold
...
`--gg-pq-hold` is **entirely derived** — there is no authored preference. It is published per layout
by `publishPqMetrics()` as the whole gap between the quote's bottom edge and the next section's top,
so the hold spends all of it and the two land **flush**:

```
hold = (1 − pqAppearTailT) · tail        // next section's top at the reveal
...
- **`--gg-pq-hold` defaults to `0vh`.** Before the first `doLayout` publishes it — or if the JS never
...
`--gg-optical-center + --gg-pq-hold` past it, and since `--gg-pq-hold` is the whole remaining gap the
rail un-sticks exactly as the next section's top reaches the quote's bottom edge.
...
| 392 | 0.519 | `zoomT ≥ pqAppearZoomT` (md 0.2904) | **md**: same, one card-shell radius later, controls **and the hint text's zero** included; next section's top is 128vh down the viewport. Both breakpoints then hold centred for `--gg-pq-hold` — the draw and the copy play out over 700ms from the cue, and the rest of the pin is still — and un-stick at its end | `updatePullQuote` + CSS, `controls.update` |
...
The **hold** (`--gg-pq-hold`) pins the rail — see **The hold, and why its length
...
(`--gg-pq-hold` 0) plays exactly like any other. On the way out the rules un-draw and the lines drop
...
| `ZOOM_TO_TAIL_T` | ratio | converts a `zoomT` fraction into a fraction of the whole **tail**, which is the space the CSS pin and `--gg-pq-hold` reason in. `1` while `PROGRESS_ZOOM_END` is `1`; applied at the one boundary (`publishPqAppearZoomT`) so the pin can't quietly release at the wrong scroll position if it ever isn't |
