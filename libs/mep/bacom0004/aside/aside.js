...
let isMobileEditorialCard = false;
...
function decorateLayout(el) {
  ...
  let mobileEditorialCardBg;
  if (isMobileEditorialCard && elems.length >= 3) {
    mobileEditorialCardBg = elems[0].innerText.trim();
    elems[0].remove();
    el.style.backgroundColor = mobileEditorialCardBg;
  }
  elems = el.querySelectorAll(':scope > div');
  if (elems.length > 1) {
...
export default function init(el) {
  ...
  isMobileEditorialCard = el.classList.contains('mobile-editorial-card');
