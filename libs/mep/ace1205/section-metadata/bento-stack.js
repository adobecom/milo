...
export default function initBentoStack(section) {
  ...
  // instead of a broken pile. CSS uses :where(.bento-stack-ready) so specificity
  // is unchanged and the reduced-motion / no-view() fallbacks still override.
  ...
  let measuring = false;
  const update = () => {
    // Ignore the ResizeObserver callbacks that our own measure() triggers
