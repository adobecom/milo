...
| `softwareapplication-default-offer` | error | When a `SoftwareApplication` (or subtype) is the primary entity of the page and has no `offers` property (or an empty array), the manager synthesizes a default free Offer (`{ "@type": "Offer", "@id": "{canonicalPageURL}#offer", "price": "0", "priceCurrency": "USD", "availability": "https://schema.org/InStock" }`) and sets `SoftwareApplication.offers` to `[{ "@id": "<canonical-offer-id>" }]`. Source for the synthesized Offer is `generated`. Producers that need a non-free Offer (paid-only SA pages, distinct paid + trial offers) supply their own; the default is only synthesized when `offers` is entirely absent. Google's rich-result spec requires `offers.price` *unconditionally* on SoftwareApplication — both `offers` and one of (`aggregateRating`, `review`) must be present for the Software App rich result to render. The free-Offer default reflects the Adobe.com norm: products are gateway-free with paid tiers. |
