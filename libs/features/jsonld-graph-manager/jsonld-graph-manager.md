...
| `ignore-types-bypass` | info | When the URL query parameter `jsonld-graph-manager-ignore` is set to a non-empty comma-separated list, the manager bypasses every producer script whose content matches an entry in the list. Matching works two ways: (a) the pseudo-type `graph` matches when the script contains any `@graph` wrapper at the top level (directly, or as an element of a top-level array); (b) a schema.org type name (compared lowercased) matches when that `@type` appears anywhere in the script's recursively-flattened nodes — at the top level, inside a `@graph` wrapper, or in nested wrappers at any depth (same recursion as ingestion's flattening). Bypassed scripts are NOT parsed for ingestion, NOT removed from the DOM, and do NOT contribute to the managed graph — they remain on the page exactly as the producer emitted them. The manager's own output script is distinguishable via the `data-milo-jsonld="graph"` attribute, so consumers can always tell the managed graph apart from a bypassed producer script. The list is empty by default; absent or empty list = nothing is bypassed. The singleton and linking rules in §3.4 / §3.5 apply only to the managed graph; ignored scripts on the page are independent of those constraints. When a script's recursive type set contains both matched and unmatched ids, the entire script is still bypassed and a Lana warning is logged; producers should split into separate scripts or callers should add `graph` to the ignore list to bypass the wrapping container intentionally. |
