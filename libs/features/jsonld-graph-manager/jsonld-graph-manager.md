...
| `ignore-types-bypass` | info | When the URL query parameter `jsonld-graph-manager-ignore` is set to a non-empty comma-separated list, the manager bypasses every producer script whose top-level content matches an entry in the list. The script's content is normalized to a list of top-level *items* — the parsed JSON itself if it is an object, or its elements if it is an array. Each item contributes zero, one, or two *match ids*: (a) the pseudo-type `graph` if the item has an `@graph` property; (b) the lowercase value of the item's `@type` (string only — arrays are not split). A script is bypassed when at least one match id is present in the ignore list. Bypassed scripts are NOT parsed for ingestion, NOT removed from the DOM, and do NOT contribute to the managed graph — they remain on the page exactly as the producer emitted them, and remain distinguishable from the manager's output via the `data-milo-jsonld="graph"` marker. The list is empty by default; absent or empty list = nothing is bypassed. The singleton and linking rules in §3.4 / §3.5 apply only to the managed graph; ignored scripts on the page are independent of those constraints. When a script's match ids are mixed — some on the ignore list and some not — the entire script is still bypassed and a Lana warning is logged; producers should split into separate scripts or callers should use `graph` to bypass the wrapping container intentionally. |
