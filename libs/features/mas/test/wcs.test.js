...
describe('resolveOfferSelectors', () => {
    it('falls into fetch-by-one strategy if Wcs responds with 404 to a multi-osi request', async () => {
        ...
        expect(results[1].reason.message).to.contain('Commerce offer not found: 404, url: https://www.adobe.com/web_commerce_artifact?offer_selector_ids=abm%2Cno-offer%2Cstock-abm%2Cvoid&country=undefined&locale=undefined_undefined&landscape=PUBLISHED&api_key=wcms-commerce-ims-ro-user-milo&language=MULT');
        expect(results[2].status).to.equal('fulfilled');
        expect(results[3].status).to.equal('rejected');
        expect(results[3].reason.message).to.contain('Commerce offer not found: 404, url: https://www.adobe.com/web_commerce_artifact?offer_selector_ids=abm%2Cno-offer%2Cstock-abm%2Cvoid&country=undefined&locale=undefined_undefined&landscape=PUBLISHED&api_key=wcms-commerce-ims-ro-user-milo&language=MULT');
    });
