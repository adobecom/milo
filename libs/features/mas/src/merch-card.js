...
export class MerchCard extends LitElement {
  static properties = {
    ...
    filters: {
      ...
      reflect: true,
      converter: {
        fromAttribute: (value) => Object.fromEntries(
          value.split(',').map((filter) => {
            ...
            return [
              key,
              {
                order: Number.isNaN(Number(value)) ? undefined : value,
                size,
              },
            ];
          }),
        ),
        toAttribute: (value) => Object.entries(value)
          .map(([key, { order, size }]) => [key, order, size]
  ...
  updateFilters(singleApp) {
    ...
    Object.keys(newFilters).forEach((key) => {
      ...
      if (value === 1 || Number.isNaN(Number(value))) return;
