...
function CheckoutMixin(Base) {
  return class CheckoutBase extends Base {
    ...
    get marketSegment() {
      const value = this.options?.ms ?? this.value?.[0]?.marketSegment;
...
function buildCheckoutUrl(checkoutData) {
  ...
  const { env, items, workflowStep, ms, cs, marketSegment, customerSegment, ot, offerType, pa, productArrangementCode, landscape, modal, is3in1, ...rest } = checkoutData;
...
var Plans = class extends VariantLayout {
  ...
  async adjustLegal() {
    ...
    const legalPromises = prices.map(async (price2) => {
      ...
      if (price2 === headingPrice) this.legal = legal2;
      await price2.onceSettled();
...
var _log, _rawData, _data, _stale, _startMark, _service2, _fragmentId, _fetchPromise, _author, _AemFragment_instances, fail_fn;
