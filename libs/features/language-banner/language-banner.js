...
async function showBanner(market, config) {
  ...
  banner.querySelector('.language-banner-close').addEventListener('click', () => {
    ...
    if (config.lingoProjectSuccessLogging === 'on') {
      window.lana.log(`Close: ${eventName}|locale:${config.locale.prefix?.replace('/', '') || 'us'}|country:${getCountry()}`, { sampleRate: 100, tags: 'lingo, lingo-language-banner-close' });
    }
    banner.remove();
  });
  ...
  if (config.lingoProjectSuccessLogging === 'on') {
    window.lana.log(`Load: ${eventName}|locale:${config.locale.prefix?.replace('/', '') || 'us'}|country:${getCountry()}`, { sampleRate: 100, tags: 'lingo, lingo-language-banner-load' });
  }
}
