...
async function createPreviewPill(manifests, overlay, utils) {
  ...
  manifests.forEach((manifest) => {
    ...
    manifestList += `<div class="hlx-manifest-info">
      <div class="hlx-manifest-title">
        ${targetTitle}
        <a class="hlx-edit-manifest" href="${manifest.manifest}" target="_blank" title="Open manifest">
  ...
  div.innerHTML = `
    <div class="hlx-manifest hlx-badge">${manifests.length} Manifest(s) served <span class="hlx-open"></span></div>
    <div class="hlx-popup">
    <div class="hlx-popup-header">
      <div>
        <h4>${manifests.length} Manifest(s) served</h4>
        <span class="hlx-close"></span>
        <div>Target integration feature is ${targetOnText}</div>
        <div>Personalization feature is ${personalizationOnText}</div>
      </div>
    </div>
    <div class="hlx-manifest-list">
      <div class="hlx-manifest-info">
        <div class="hlx-manifest-variants"><div>
  ...
  const radioInputs = div.querySelectorAll('.hlx-popup input[type="radio"]');
  ...
  const checkbox = div.querySelectorAll('.hlx-popup input[type="checkbox"]');
  ...
  const textInput = div.querySelectorAll('.hlx-popup input[type="text"]');
  ...
  const toggle = div.querySelector('.hlx-manifest.hlx-badge');
  toggle.addEventListener('click', () => {
    div.classList.toggle('hlx-hidden');
  });

  const close = div.querySelector('.hlx-close');
  close.addEventListener('click', () => {
    document.body.removeChild(document.querySelector('.hlx-preview-overlay'));
  });
  overlay.append(div);
}
...
export async function decoratePreviewMode(manifests, utils) {
  ...
  const overlay = utils.createTag('div', { class: 'hlx-preview-overlay', style: 'display: none;' });
