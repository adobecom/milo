...
function createPreviewPill(manifests) {
  ...
  manifests.forEach((manifest) => {
    ...
    const scheduled = manifest.previewOn && manifest.previewOff
      ? `<p>Scheduled - ${manifest.previewDisabled ? 'inactive' : 'active'}</p>
        <p>On: ${manifest.previewOn.toLocaleString()}</p>
         <p>Off: ${manifest.previewOff.toLocaleString()}</p>` : '';
