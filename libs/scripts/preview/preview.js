...
const getExperimentConfig = (experiment, instantExperiment) => {
  if (instantExperiment) {
    return getConfigForInstantExperiment(experiment, instantExperiment);
  }
  return getConfigForFullExperiment(experiment);
};
...
async function createExperiment() {
  // TODO: Multiple experiments now stored in window.hlx.experiments
  ...
  if (experiment) {
    const config = await getExperimentConfig(experiment, instantExperiment);
    if (!config) return ''; // TODO: Provide error message
    if (!config.experimentName) config.experimentName = experiment;
    const createVariant = (variantName) => {
      ...
      experimentURL.searchParams.set('experiment', `${experimentId}/${variantName}`);
...
async function decoratePreviewMode() {
  loadStyle('/libs/scripts/preview/preview.css');
