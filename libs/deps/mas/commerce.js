...
var EVENT_TYPE_ERROR = "mas:commerce:error";
var EVENT_TYPE_FAILED = "mas:failed";
var EVENT_TYPE_PENDING = "mas:pending";
var EVENT_TYPE_READY = "mas:commerce:ready";
...
var MasElement = class {
  ...
  requestUpdate(force = false) {
    if (!this.wrapperElement.isConnected || !useService2())
      return;
    if (this.timer)
      return;
...
function Wcs({ settings }) {
  ...
  function flushWcsCache() {
    ...
    cache.clear();
    log2.debug(`Flushed ${size} cache entries`);
  }
...
var _MasCommerceService = class _MasCommerceService extends HTMLElement {
  ...
  refreshOffers() {
    this.flushWcsCache();
    document.querySelectorAll('span[is="inline-price"],a[is="checkout-link"]').forEach((el) => el.requestUpdate(true));
