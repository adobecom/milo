...
var escapeHtml = (html2) => `${html2 ?? ""}`.replace(
  /[&<>'"]/g,
  (tag) => ({
    "&": "&amp;",
    "<": "&lt;",
...
var renderAttributes = (attributes) => isObject(attributes) ? Object.entries(attributes).filter(
  ...
  (html2, [key, value]) => html2 + ` ${key}${value === true ? "" : '="' + escapeHtml(value) + '"'}`,
  ""
) : "";
...
var MERCH_CARD_COLLECTION_LOAD_TIMEOUT = 1e4;
var SORT_ORDER = {
...
var _overrideMap, _service2, _log, _MerchCardCollection_instances, fail_fn;
var MerchCardCollection = class extends LitElement {
  ...
  checkReady() {
    ...
    const timeoutPromise = new Promise(
      (resolve) => setTimeout(() => resolve(false), MERCH_CARD_COLLECTION_LOAD_TIMEOUT)
    );
  ...
  connectedCallback() {
    ...
    __privateSet(this, _log, __privateGet(this, _service2).Log.module(MERCH_CARD_COLLECTION));
  ...
  get filtersButton() {
    return this.sidenav && this.mobileAndTablet.matches ? html`<sp-action-button
                  id="filtersButton"
                  variant="secondary"
                  treatment="outline"
  ...
  get searchBar() {
    ...
    return searchPlaceholder && this.mobileAndTablet.matches ? html`<merch-search deeplink="search">
                  <sp-search
                      id="searchBar"
...
__publicField(MerchCardCollection, "styles", [styles]);
