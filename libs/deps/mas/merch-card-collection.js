...
`;var fe="mas-commerce-service";var M=(s,t)=>s?.querySelector(`[slot="${t}"]`)?.textContent?.trim();function te(){return document.getElementsByTagName(fe)?.[0]}var ie="merch-card-collection",_e=2e4,Ae={catalog:["four-merch-cards"],plans:["four-merch-cards"],plansThreeColumns:["three-merch-cards"]},ge={plans:!0},Re=(s,{filter:t})=>s.filter(e=>e.filters.hasOwnProperty(t)),xe=(s,{types:t})=>t?(t=t.split(","),s.filter(e=>t.some(r=>e.types.includes(r)))):s,Se=s=>s.sort((t,e)=>(t.title??"").localeCompare(e.title??"","en",{sensitivity:"base"})),Ce=(s,{filter:t})=>s.sort((e,r)=>r.filters[t]?.order==null||isNaN(r.filters[t]?.order)?-1:e.filters[t]?.order==null||isNaN(e.filters[t]?.order)?1:e.filters[t].order-r.filters[t].order),Le=(s,{search:t})=>t?.length?(t=t.toLowerCase(),s.filter(e=>(e.title??"").toLowerCase().includes(t))):s,S,P,D,I,k,oe,L=class extends se{constructor(){super();A(this,k);A(this,S,{});A(this,P);A(this,D);A(this,I);this.filter="all",this.hasMore=!1,this.resultCount=void 0,this.displayResult=!1,this.data=null,this.variant=null,this.hydrating=!1,this.hydrationReady=null,this.literalsHandlerAttached=!1,this.onUnmount=[]}render(){return T`
      ...
      `);customElements.define("merch-card-collection-header",w);export{L as MerchCardCollection,w as default};
