...
`;var Vt=class extends M{constructor(t){super(t)}getGlobalCSS(){return Zi}renderLayout(){return Ji` ${this.badge}
    ...
    ${this.card.customHr?"":Ji`<hr />`} ${this.secureLabelFooter}`}};import{html as Ve,css as Wo,unsafeCSS as ta}from"../lit-all.min.js";var ea=`
  :root {
...
`;var Yo=32,qe=class extends M{constructor(r){super(r);g(this,"getRowMinHeightPropertyName",r=>`--consonant-merch-card-footer-row-${r}-min-height`);g(this,"getMiniCompareFooter",()=>{let r=this.card.secureLabel?Ve`<slot name="secure-transaction-label">
        ...
        <slot name="footer-rows"><slot name="body-s"></slot></slot>`}async postCardUpdateHook(){await Promise.all(this.card.prices.map(r=>r.onceSettled())),await this.adjustAddon(),k.isMobile?this.removeEmptyRows():(this.adjustMiniCompareBodySlots(),this.adjustMiniCompareFooterRows())}};g(qe,"variantStyle",Wo`
...
`)[0].trim()}updateShortDescriptionVisibility(){let t=this.card.querySelector('[slot="short-description"]');if(!t)return;let r=t.querySelector("strong, b, p");r&&(k.isDesktopOrUp?r.style.display="":r.style.display="none")}toggleShortDescription(){this.shortDescriptionExpanded=!this.shortDescriptionExpanded,this.card.requestUpdate()}get shortDescriptionToggle(){return this.hasShortDescription?k.isDesktopOrUp?ke`
...
`;var We=class extends M{constructor(t){super(t)}getGlobalCSS(){return la}postCardUpdateHook(){this.adjustTitleWidth()}renderLayout(){return Yr` ${this.badge}
    <div class="body">
        <slot name="heading-xs"></slot>
        <slot name="body-xxs"></slot>
...
`;var pa={name:{tag:"h4",slot:"detail-m"},title:{tag:"h4",slot:"detail-m"},backgroundImage:{tag:"div",slot:"bg-image"},prices:{tag:"h3",slot:"heading-xs"},description:{tag:"div",slot:"body-xs"},ctas:{slot:"footer",size:"l"}},Ye=class extends M{constructor(t){super(t)}getGlobalCSS(){return da}get headingSelector(){return'[slot="detail-m"]'}renderLayout(){return Xr`${this.cardImage}
            <div class="body">
