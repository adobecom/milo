...
`,Ua=()=>{let e=[$`
    ...
    `),e};var zt=class zt{constructor(t){g(this,"card");this.card=t,this.insertVariantStyle()}insertVariantStyle(){if(!zt.styleMap[this.card.variant]){zt.styleMap[this.card.variant]=!0;let t=document.createElement("style");t.innerHTML=this.getGlobalCSS(),document.head.appendChild(t)}}get badge(){let t;if(!(!this.card.badgeBackgroundColor||!this.card.badgeColor||!this.card.badgeText))return this.evergreen&&(t=`border: 1px solid ${this.card.badgeBackgroundColor}; border-right: none;`),x`
        <div
            id="badge"
...
merch-card[variant="catalog"] .payment-details {
  ...
  line-height: var(--consonant-merch-card-body-line-height);
    ...
    <slot></slot>`}getGlobalCSS(){return Fa}connectedCallbackHook(){this.card.addEventListener("mouseleave",this.toggleActionMenu)}disconnectedCallbackHook(){this.card.removeEventListener("mouseleave",this.toggleActionMenu)}};g(ct,"variantStyle",$`
...
`;var lt=class extends R{constructor(t){super(t)}getGlobalCSS(){return ja}renderLayout(){return x` <div class="body">
        <slot name="icons"></slot> ${this.badge}
        <slot name="heading-xs"></slot>
        <slot name="heading-m"></slot>
...
`;var Fr=class extends R{constructor(t){super(t)}getGlobalCSS(){return Ka}renderLayout(){return x`${this.cardImage}
    <div class="body">
...
`;var jr=class extends R{constructor(t){super(t)}getGlobalCSS(){return Ba}renderLayout(){return x` ${this.badge}
    <div class="body">
        <div class="top-section">
            <slot name="icons"></slot>
            <slot name="heading-xs"></slot>
...
`;var El=32,ut,ht=class extends R{constructor(r){super(r);q(this,ut);g(this,"getRowMinHeightPropertyName",r=>`--consonant-merch-card-footer-row-${r}-min-height`);g(this,"getMiniCompareFooter",()=>{let r=this.card.secureLabel?x`<slot name="secure-transaction-label">
        ...
        <slot name="footer-rows"><slot name="body-s"></slot></slot>`}async postCardUpdateHook(){zr()?this.removeEmptyRows():(await Promise.all(this.card.prices.map(r=>r.onceSettled())),this.adjustMiniCompareBodySlots(),this.adjustMiniCompareFooterRows())}};ut=new WeakMap,g(ht,"variantStyle",$`
...
`;var mt=class extends R{constructor(t){super(t)}getGlobalCSS(){return Xa}postCardUpdateHook(){this.adjustTitleWidth()}get stockCheckbox(){return this.card.checkboxLabel?x`<label id="stock-checkbox">
        ...
        ${this.secureLabelFooter}`}};g(mt,"variantStyle",$`
    :host([variant='plans']) {
      min-height: 348px;
    }
...
`;var dt,ze=class extends R{constructor(r){super(r);q(this,dt)}getContainer(){return Z(this,dt,G(this,dt)??this.card.closest('[class*="-merch-cards"]')??this.card.parentElement),G(this,dt)}getGlobalCSS(){return Wa}updateCardElementMinHeight(r,n){let i=`--consonant-merch-card-product-${n}-height`,o=Math.max(0,parseInt(window.getComputedStyle(r).height)||0),a=parseInt(this.getContainer().style.getPropertyValue(i))||0;o>a&&this.getContainer().style.setProperty(i,`${o}px`)}adjustProductBodySlots(){if(this.card.getBoundingClientRect().width===0)return;["heading-xs","body-xxs","body-xs","promo-text","callout-content","body-lower"].forEach(n=>this.updateCardElementMinHeight(this.card.shadowRoot.querySelector(`slot[name="${n}"]`),n))}renderLayout(){return x` ${this.badge}
      <div class="body">
          <slot name="icons"></slot>
          <slot name="heading-xs"></slot>
          <slot name="body-xxs"></slot>
      ...
      ${this.secureLabelFooter}`}postCardUpdateHook(){zr()||this.adjustProductBodySlots()}};dt=new WeakMap,g(ze,"variantStyle",$`
...
`;var pt=class extends R{constructor(t){super(t)}getGlobalCSS(){return qa}postCardUpdateHook(){this.adjustTitleWidth()}renderLayout(){return x` ${this.badge}
    <div class="body">
        <slot name="heading-xs"></slot>
        <slot name="body-xxs"></slot>
        ${this.promoBottom?"":x`<slot name="promo-text"></slot><slot name="callout-content"></slot>`}
    ...
    <hr />
    ${this.secureLabelFooter}`}};g(pt,"variantStyle",$`
...
`;var ft=class extends R{constructor(t){super(t)}getGlobalCSS(){return Za}get headingSelector(){return'[slot="detail-m"]'}renderLayout(){return x`${this.cardImage}
          <div class="body">
...
`;var gt=class extends R{constructor(t){super(t)}getGlobalCSS(){return Ja}renderLayout(){return x`${this.badge}
      <div class="top-section">
          <slot name="icons"></slot>
          <slot name="heading-xs"></slot>
      ...
      <footer><slot name="footer"></slot></footer>`}};g(gt,"variantStyle",$`
    :host([variant='twp']) {
      padding: 4px 10px 5px 10px;
    }
    .twp-badge {
  ...
  `);var Ii=(e,t=!1)=>{switch(e.variant){case"catalog":return new ct(e);case"ccd-action":return new lt(e);case"image":return new Fr(e);case"inline-heading":return new jr(e);case"mini-compare-chart":return new ht(e);case"plans":return new mt(e);case"product":return new ze(e);case"segment":return new pt(e);case"special-offers":return new ft(e);case"twp":return new gt(e);default:return t?void 0:new ze(e)}},Qa=()=>{let e=[];return e.push(ct.variantStyle),e.push(lt.variantStyle),e.push(ht.variantStyle),e.push(ze.variantStyle),e.push(mt.variantStyle),e.push(pt.variantStyle),e.push(ft.variantStyle),e.push(gt.variantStyle),e};var es=document.createElement("style");es.innerHTML=`
:root {
