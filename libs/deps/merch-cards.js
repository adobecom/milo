...
`;var d=(r,t)=>r.querySelector(`[slot="${t}"]`).textContent.trim();var B="merch-cards",l={alphabetical:"alphabetical",authored:"authored"};var L={filters:[,"resultText","resultsText"],mobile:["noResultsMobileText","searchResultMobileText","searchResultsMobileText"],desktop:["noResultsText","searchResultText","searchResultsText"]},M=(r,{filter:t})=>r.filter(e=>e.filters.hasOwnProperty(t)),k=(r,{types:t})=>t?(t=t.split(","),r.filter(e=>t.some(i=>e.types.includes(i)))):r,$=r=>r.sort((t,e)=>(t.title??"").localeCompare(e.title??"","en",{sensitivity:"base"})),R=(r,{filter:t})=>r.sort((e,i)=>i.filters[t]?.order==null||isNaN(i.filters[t]?.order)?-1:e.filters[t]?.order==null||isNaN(e.filters[t]?.order)?1:e.filters[t].order-i.filters[t].order),_=(r,{search:t})=>t?.length?(t=t.toLowerCase(),r.filter(e=>e.includes(t))):r,m=class extends A{static properties={filter:{type:String,attribute:"filter",reflect:!0},filtered:{type:String,attribute:"filtered"},search:{type:String,attribute:"search",reflect:!0},sort:{type:String,attribute:"sort",default:l.authored,reflect:!0},types:{type:String,attribute:"types",reflect:!0},limit:{type:Number,attribute:"limit"},page:{type:Number,attribute:"page",reflect:!0},singleApp:{type:String,attribute:"single_app"},hasMore:{type:Boolean},resultCount:{type:Number}};#e;#t;mobileAndTablet=new u(this,y);constructor(){super(),this.filter="all",this.hasMore=!1,this.resultCount=0,this.#t=0}render(){return a`${this.header}
            <slot></slot>
            ${this.footer}`}updated(t){let e=[...this.children].filter(s=>s.tagName==="MERCH-CARD");t.has("singleApp")&&this.singleApp&&e.forEach(s=>{s.updateFilters(s.name===this.singleApp)});let i=this.sort===l.alphabetical?$:R,o=[M,k,_,i].reduce((s,c)=>c(s,this),e).map((s,c)=>[s,c]);if(this.resultCount=o.length,this.page&&this.limit){let s=this.page*this.limit;this.hasMore=o.length>s,o=o.filter(([,c])=>c<s)}let h=new Map(o);this.#t=(this.limit*(this.page-1)+1).toString(),e.forEach(s=>{h.has(s)?(s.style.order=h.get(s),s.style.order===this.#t&&(this.#e=s),s.size=s.filters[this.filter]?.size,s.style.removeProperty("display")):(s.style.display="none",s.size=void 0,s.style.removeProperty("order"))}),this.updateComplete.then(()=>{let s=this.shadowRoot.getElementById("resultText")?.firstElementChild?.assignedElements?.()?.[0];s&&x(s,{resultCount:this.resultCount,searchTerm:this.search,filter:this.sidenav?.filters.selectedText})})}connectedCallback(){super.connectedCallback(),this.filtered?this.filter=this.filtered:this.startDeeplink()}disconnectedCallback(){super.disconnectedCallback(),this.stopDeeplink?.()}get header(){if(!this.filtered)return a`<div id="header">
