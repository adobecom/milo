// Fri, 24 Nov 2023 13:55:23 GMT
import{html as a,LitElement as B}from"./lit-all.min.js";import{unsafeHTML as E}from"./lit-all.min.js";var u=class{constructor(t,e){this.key=Symbol("match-media-key"),this.matches=!1,this.host=t,this.host.addController(this),this.media=window.matchMedia(e),this.matches=this.media.matches,this.onChange=this.onChange.bind(this),t.addController(this)}hostConnected(){var t;(t=this.media)==null||t.addEventListener("change",this.onChange)}hostDisconnected(){var t;(t=this.media)==null||t.removeEventListener("change",this.onChange)}onChange(t){this.matches!==t.matches&&(this.matches=t.matches,this.host.requestUpdate(this.key,!this.matches))}};var f="hashchange";function v(r=window.location.hash){let t=[],e=r.replace(/^#/,"").split("&");for(let o of e){let[n,i=""]=o.split("=");n&&t.push([n,decodeURIComponent(i)])}return Object.fromEntries(t)}function p(r){let t=new URLSearchParams(window.location.hash.slice(1));Object.entries(r).forEach(([e,o])=>{o?t.set(e,o):t.delete(e)}),t.sort(),window.location.hash=decodeURIComponent(t.toString())}function g(r){let t=e=>{let o=v(window.location.hash);r(o)};return t(),window.addEventListener(f,t),()=>{window.removeEventListener(f,t)}}var T=(r,t={})=>{r.querySelectorAll("span[data-placeholder]").forEach(e=>{let{placeholder:o}=e.dataset;e.innerText=t[o]??""})};var x="(max-width: 1200px)",y="(min-width: 900px)",b="(min-width: 1200px)";import{css as C,unsafeCSS as S}from"./lit-all.min.js";var w=C`
    #header,
    #resultText,
    #footer {
        grid-column: 1 / -1;
...
`;var d=(r,t)=>r.querySelector(`[slot="${t}"]`).textContent.trim();var M="merch-cards",l={alphabetical:"alphabetical",authored:"authored"},$={filters:[,"resultText","resultsText"],mobile:["noResultsMobileText","searchResultMobileText","searchResultsMobileText"],desktop:["noResultsText","searchResultText","searchResultsText"]},A=(r,{filter:t})=>r.filter(e=>e.filters.hasOwnProperty(t)),k=(r,{types:t})=>t?(t=t.split(","),r.filter(e=>t.some(o=>e.types.includes(o)))):r,L=r=>r.sort((t,e)=>(t.title??"").localeCompare(e.title??"","en",{sensitivity:"base"})),N=(r,{filter:t})=>r.sort((e,o)=>o.filters[t]?.order==null||isNaN(o.filters[t]?.order)?-1:e.filters[t]?.order==null||isNaN(e.filters[t]?.order)?1:e.filters[t].order-o.filters[t].order),R=(r,{search:t})=>t?.length?(t=t.toLowerCase(),r.filter(e=>e.includes(t))):r,m=class extends B{static properties={filter:{type:String,attribute:"filter",reflect:!0},filtered:{type:String,attribute:"filtered"},search:{type:String,attribute:"search",reflect:!0},sort:{type:String,attribute:"sort",default:l.authored,reflect:!0},types:{type:String,attribute:"types",reflect:!0},limit:{type:Number,attribute:"limit"},page:{type:Number,attribute:"page",reflect:!0},singleApp:{type:String,attribute:"single_app"},hasMore:{type:Boolean},resultCount:{type:Number}};mobileAndTablet=new u(this,x);constructor(){super(),this.filter="all",this.hasMore=!1,this.resultCount=0}render(){return a`${this.header}
            <slot></slot>
            ${this.footer} `}updated(t){let e=[...this.children].filter(s=>s.tagName==="merch-card");t.has("singleApp")&&this.singleApp&&e.forEach(s=>{s.updateFilters(s.name===this.singleApp)});let o=this.sort===l.alphabetical?L:N,i=[A,k,R,o].reduce((s,c)=>c(s,this),e).map((s,c)=>[s,c]);if(this.resultCount=i.length,this.page&&this.limit){let s=this.page*this.limit;this.hasMore=i.length>s,i=i.filter(([,c])=>c<s)}i.length>0&&(this.cardToScrollTo=i[i.length-1][0]);let h=new Map(i);e.forEach(s=>{h.has(s)?(s.style.order=h.get(s),s.size=s.filters[this.filter]?.size,s.style.removeProperty("display")):(s.style.display="none",s.size=void 0,s.style.removeProperty("order"))}),this.updateComplete.then(()=>{let s=this.shadowRoot.getElementById("resultText")?.firstElementChild?.assignedElements?.()?.[0];s&&T(s,{resultCount:this.resultCount,searchTerm:this.search})})}connectedCallback(){super.connectedCallback(),this.filtered?this.filter=this.filtered:this.startDeeplink(),this.updateComplete.then(()=>{this.prepareShowMore()})}disconnectedCallback(){super.disconnectedCallback(),this.stopDeeplink?.()}get header(){if(!this.filtered)return a`<div id="header">
        ...
        >
            <slot name="showMoreText"></slot>
              ...
              ></sp-button>`:""}get searchBar(){let t=d(this,"searchText");return this.mobileAndTablet.matches?a` <sp-search
                  id="searchBar"
                  placeholder="${t}"
                  ></sp-button>`:""}get sortButton(){if(this.resultCount<=1)return;let t=d(this,"sortText"),e=d(this,"popularityText"),o=d(this,"alphabeticallyText");if(!(t&&e&&o))return;let n=this.sort===l.alphabetical,i=n?"":"selected",h=n?"selected":"";return a`
