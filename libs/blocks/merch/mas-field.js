...
// merch-card or its dependencies, so LCP blocks (e.g. marquee) authored with a field-only
// link stay off that critical path.
...
async function loadFieldDependencies() {
  ...
  log = service.Log.module('mas-field');
  await loadMasComponent(MAS_FIELD);
}

async function checkReady(masField, fragment) {
  if (isMasErrorEnv()) {
  ...
  if (success === 'timeout') {
    log.error(`${masField.tagName} did not initialize withing give timeout`);
  } else if (!success) {
    log.error(`${masField.tagName} failed to initialize`);
  }
}
...
export default async function initMasField(el) {
  watchMasFieldCtas();
