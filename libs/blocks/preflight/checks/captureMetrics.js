...
const getLogsEndpoint = () => {
  ...
  const env = searchParams.get('preflightlogs');
  if (env === 'local') return 'http://localhost:8080/preflight-logs';
  if (env === 'stage') return 'https://milo-core-stage.adobe.io/preflight-logs';
...
async function capture(results) {
  ...
  const videoPosterCheck = resolvedPerformance?.find((c) => (c.checkId || c.id) === 'video-poster');
  const performanceVideoAsLcp = videoPosterCheck?.metricsExtra?.performance_video_as_lcp ?? null;
