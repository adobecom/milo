...
export default async function collectFragmentChanges(root, pageUrl) {
  ...
  await Promise.all([...byPath].map(async ([path, els]) => {
    try {
      const url = new URL(path, pageUrl.origin);
      const { content } = computeDiff(await fetchVersions(url));
      if (!content) return;
      els.forEach((scope) => {
        content.added.forEach((change) => added.push({ ...change, scope }));
        content.modified.forEach((change) => modified.push({ ...change, scope }));
      });
    } catch (e) {
