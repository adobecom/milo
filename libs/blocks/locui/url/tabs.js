...
function Actions({ item }) {
  ...
  const disable = itemUrl?.valid !== undefined && !itemUrl.valid;
  return html`
    <div class=locui-url-source-actions>
      <button
        disabled=${item.value.edit?.status === 404}
        class="locui-url-action locui-url-action-edit${isExcel}${disable ? ' disabled' : ''}"
        onClick=${(e) => { if (!disable) openWord(e, item); }}>Edit</button>
      <button
        class="locui-url-action locui-url-action-view${isDisabled(item.value.preview?.status)}"
        onClick=${(e) => { if (!disable) handleAction(e, item, true); }}>Preview</button>
      <button
        class="locui-url-action locui-url-action-view${isDisabled(item.value.live?.status)}"
        onClick=${(e) => { if (!disable) handleAction(e, item); }}>Live</button>
    </div>
  `;
}
