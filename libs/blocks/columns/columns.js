...
const isRow = (cols, isTable) => isTable && Array.from(cols).some((col) => col.hasChildNodes());

const applyAccessibilityAttributes = (col, rdx, cols, cdx, rows) => {
...
export default function init(el) {
  ...
  rows.forEach((row, rdx) => {
    ...
    if (isRow(cols, isTable)) row.setAttribute('role', 'row');
    ...
    cols.forEach((col, cdx) => {
      ...
      applyAccessibilityAttributes(col, rdx, cols, cdx, rows);
    });
  });
}
