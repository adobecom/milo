...
const MenuItem = ({li, i}) => html`<li><a href=${makeRelative(li.firstChild.href, true)} daa-ll=${`${analyticsGetLabel(li.textContent)}-${i + 1}`}>${li.textContent}</a></li>`

const GnavPromo = ({div}) => {
  const anchorTags = div.querySelectorAll("a")
  for(const anchorTag of anchorTags) {
    anchorTag.setAttribute("daa-ll", `${analyticsGetLabel(anchorTag.textContent)}-${childIndexOf(anchorTag.parentElement)+1}`)
  }
  // TODO dangerouslySetInnerHTML? Alternative? Sanitize?
  return html`<div class=${Array.from(div.classList).join(" ")} dangerouslySetInnerHTML=${{__html: div.innerHTML}}></div>`
}
...
const NavMenu = ({navLink, id, menu}) => {
  ...
  return html`
    <div id=${id} class="gnav-navitem-menu${small || medium || large}" daa-lh=${`header|${navLink.textContent}`}>
      ${menu.childElementCount >= 4 && html`<div class="gnav-menu-container"></div>`}
      ${Array.from(menu.children).map((child => {
        if(child.nodeName === "UL") {
          return html`<ul>${Array.from(child.children).map((li, i) => MenuItem({li, i}))}</ul>`
        }
...
const NavLink = ({navLink, index, menuOpen, setMenuOpen}) => {
  ...
  if(!hasMenu) {
    return html`
      <div class="gnav-navitem">
        <a href=${makeRelative(navLink.href, true)} role="button">${navLink.textContent}</a>
      </div>
    `
  }

  const largeMenu = navBlock ? ' large-menu' : ''
  const sectionMenu = navBlock?.classList.contains('section') ? ' section-menu' : ''
  ...
  return html`
    <div class="gnav-navitem${hasMenu}${largeMenu}${sectionMenu}${menuOpen === index ? ' is-open' : ''}" >
      <a 
        onClick=${handleClick} 
      ...
      ${menu.childElementCount > 1 && html`<${NavMenu} id=${`navmenu-${index}`} menu=${menu} navLink=${navLink}/>`}
...
const Search = ({body, searchOpen, setSearchOpen}) => {
  ...
  const noResults = searchInput 
    ...
    && html`
      <li>
        <a href=${getHelpxLink(searchInput)} aria-label="Try our advanced search">
...
const MainNav = ({body, menuOpen, setMenuOpen, searchOpen, setSearchOpen}) => {
  const mainLinks = body.querySelectorAll('h2 > a');
  return html`
    <div class="mainnav-wrapper">
      <div class="gnav-mainnav">
        ${mainLinks && [...mainLinks].map((navLink, index) => html`
          <${NavLink} navLink=${navLink} index=${index} menuOpen=${menuOpen} setMenuOpen=${setMenuOpen}/>
        `)}
        <${Cta} body=${body}/>
      </div>
      <${Search} body=${body} searchOpen=${searchOpen} setSearchOpen=${setSearchOpen}/>
    </div>
  `
}
...
const Logo = ({body}) => {
  ...
  return html`
    <a 
      ...
      daa-ll="Logo">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 133.46 118.11"><defs><style>.cls-1{fill:#fa0f00;}</style></defs><polygon class="cls-1" points="84.13 0 133.46 0 133.46 118.11 84.13 0"></polygon><polygon class="cls-1" points="49.37 0 0 0 0 118.11 49.37 0"></polygon><polygon class="cls-1" points="66.75 43.53 98.18 118.11 77.58 118.11 68.18 94.36 45.18 94.36 66.75 43.53"></polygon></svg>
    </a>
  `
}
...
const GnavPreact = ({name, body}) => {
  ...
  return html`
    <header class="gnav${gnavOpen ? ` ${IS_OPEN}` : ''}" daa-im="true" daa-lh="gnav${name}">
      <div class="gnav-curtain${menuOpen ? ' is-open is-quiet': ''}${searchOpen ? ' is-open' : ''}"></div>
      <div class="gnav-wrapper">
        <nav class="gnav" aria-label="Main">
          <button class="gnav-toggle" aria-label="Navigation menu" aria-expanded=${gnavOpen} onClick=${() => setGnavOpen(!gnavOpen)}></button>
          <${BrandBlock} body=${body}/>
          <${MainNav} body=${body} menuOpen=${menuOpen} setMenuOpen=${setMenuOpen} searchOpen=${searchOpen} setSearchOpen=${setSearchOpen}/>
          <${Profile} body=${body}/>
          <${Logo} body=${body}/>
        </nav>
      </div>
...
export default async function init(header) {
  ...
  try {
    ...
    const gnavPreact = html`<${GnavPreact} body=${gnavDoc.body} name=${name}/>`
