...
// Makes the Heading tags look right
...
function getItem(title, description, target) {
  ...
  const linkText = createTag('div', {
   ...
   if (description) linkText.append(createTag('p', {
     class: 'section-description'
   }, description));
   item.append(linkText);
   if (target) item.append(createTag('div', {
     class: 'toc-arrow'
   }, DOWN_ARROW_ICON));
...
export default function init(el) {
 ...
 // console.table(el);
  ...
  const tocMarquee = createTag('div', {class: 'marquee light medium'}, `<div class="foreground container">${title}</div>`);
  ...
  //const headline = foreground.querySelector('h1, h2, h3, h4, h5, h6');
  //const text = headline.closest('div');
  //const text = headline.nextElementSibling();

  tocMarquee.setAttribute('daa-lh', 'marquee|light|medium');
  const tocContainer = createTag('div', {
    class: 'container toc-container'
  }, `<p class="toc-title">${title}</p>`);
  const tocNav = createTag('nav', {
    'aria-label': 'Table of contents'
  });
  const navUl = createTag('ul', {
    class: 'toc-list'
  });
  ...
  tocNav.append(navUl);
  tocContainer.append(foreground);
  tocContainer.append(tocNav);
  el.replaceChildren(tocMarquee);
  el.append(tocContainer);
}
