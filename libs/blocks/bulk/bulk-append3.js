...
function StatusContent(props) {
  useEffect(async () => {
    ...
    const actions = props.submittedAction?.split('&');
    if (!urls || !actions) return;
    const rowsElt = document.querySelector('.bulk-status-rows');
    rowsElt.innerHTML = '';
    if (!rowsElt) return;
    for (let i = 0; i < urls.length; i++) {
      const url = urls[i];
      ...
      const rowHtml = `
              <div class="bulk-status-row">
                  <div class="bulk-status-url">${status.preview ? url : ''}</div>
                  <div class="bulk-status-preview">${status.preview ? status.preview : ''}</div>
                  <div class="bulk-status-url">${status.live ? url : ''}</div>
                  <div class="bulk-status-publish">${status.live ? status.live : ''}</div>
              </div>`;
      const rowElt = createTag('div', null, rowHtml);
      rowsElt.append(rowElt);
    }
  }, [props]);
  const timeStamp = prettyDate();
  return html`
    <div class="bulk-status-content">
        ${props.submittedAction && html`
            ...
            <div class="bulk-status-footer">
                <div class="bulk-status-footer-preview">
                    <div class="bulk-status-footer-preview-complete">${html`job complete`}</div>
                    <div class="bulk-status-footer-preview-date">${timeStamp}</div>
                </div>
                <div class="bulk-status-footer-publish">
                    <div class="bulk-status-footer-publish-complete">${html`job complete`}</div>
