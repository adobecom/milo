...
async function getShowMoreButtonContent() {
  return `${await replacePlaceholder('see-more-features')} ${ADD_MORE_ICON}`;
}
...
async function handleShowMoreButton(section) {
  ...
  showMoreBtn.innerHTML = await getShowMoreButtonContent();
...
const init = async (el) => {
  ...
  // ##mweb Add show more button for mobile view
