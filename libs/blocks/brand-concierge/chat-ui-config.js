...
/** Token receiver registered by the script via bridge.setTokenReceiver(); called from onSuccessfulToken. */
...
/** Window property name for the host bridge; passed to bootstrap so the script can look up the bridge. */
 ...
 * Trigger via URL param ?bc-local=1 or ?brand-concierge-local=1, or metadata "brand-concierge-local".
 */
function useLocalBrandConciergeWebAgent() {
...
async function openChatModal(initialMessage, el) {
  ...
  if (useLocalBrandConciergeWebAgent()) {
...
function createSusiComponentForModal({
  ...
  susi.authParams = { ...authParams, redirect_uri: destURL, dctx_id: isStage ? DCTX_ID_STAGE : DCTX_ID_PROD };
  susi.config = config;
...
async function openSusiLightModal() {
  ...
  /* Popup mode (Nest/Express): auth runs in a separate window; modal closes on redirect or on-token */
  const usePopup = true;
