  ...
  [...images].forEach((img) => {
    ...
    try {
      ...
      if (href.includes(".mp4")) {
        ...
        if (icon) {
          import("./image-video-link-BRLP52J3.js").then((mod) => mod.default(picParent, aTag, icon));
        } else {
          aTag.append(pic);
        }
      }
...
async function decoratePlaceholders(area, config) {
  ...
  const { decoratePlaceholderArea } = await import("./placeholders-FYEYGHYH.js");
...
async function decorateFooterPromo(doc = document) {
  ...
  const { default: initFooterPromo } = await import("./footer-promo-QD4SIA2Y.js");
...
async function loadMartech({
  ...
  const { default: initMartech } = await import("./martech-3OADLNNK.js");
...
async function checkForPageMods() {
  ...
  const { init } = await import("./personalization-CN5N3ZMO.js");
  await init({
    mepParam,
    mepHighlight,
    mepButton,
...
async function loadPostLCP(config) {
  ...
  if (georouting === "on") {
    const { default: loadGeoRouting } = await import("./georoutingv2-NT2N3D3D.js");
  ...
  if (config?.mep) {
    import("./personalization-CN5N3ZMO.js").then(({ addMepAnalytics }) => addMepAnalytics(config, header));
  }
}
...
async function loadDeferred(area, blocks, config) {
  ...
  if (config.locale?.ietf === "ja-JP") {
    import("./japanese-word-wrap-IMJNXFTQ.js").then(({ default: controlJapaneseLineBreaks }) => {
  ...
  if (config.mep?.preview) {
    import("./preview-EJXSQ5AD.js").then(({ default: decoratePreviewMode }) => decoratePreviewMode());
  }
  if (config?.dynamicNavKey && config?.env?.name !== "prod") {
    ...
    const { default: loadDNStatus } = await import("./status-KPZL64T7.js");
    loadDNStatus();
  }
}
function initSidekick() {
  const initPlugins = async () => {
    const { default: init } = await import("./sidekick-EYVBWDOY.js");
...
function decorateMeta() {
  ...
  window.addEventListener("modal:open", async (e) => {
    ...
    const { findDetails, getModal } = await import("./modal-WBOTTJWV.js");
...
async function documentPostSectionLoading(config) {
  ...
  if (getMetadata("seotech-structured-data") === "on" || getMetadata("seotech-video-url")) {
    import("./seotech-QLR7GTZP.js").then((module) => module.default(
  ...
  import("./attributes-RVL5BN4D.js").then((analytics) => {
...
async function resolveInlineFrags(section) {
  ...
  const { default: loadInlineFrags } = await import("./fragment-WRBFU54Z.js");
...
//# sourceMappingURL=chunk-DIP3NAMX.js.map
