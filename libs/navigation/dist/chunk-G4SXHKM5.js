  ...
  [...images].forEach((img) => {
    ...
    try {
      ...
      if (href.includes(".mp4")) {
        ...
        if (icon) {
          import("./image-video-link-Q6RAWTUL.js").then((mod) => mod.default(picParent, aTag, icon));
        } else {
          aTag.append(pic);
        }
      }
...
function convertStageLinks({ anchors, config, hostname, href }) {
  ...
  const matchedRules = Object.entries(config.stageDomainsMap).find(([domain]) => new RegExp(domain).test(href));
  if (!matchedRules) return;
  const [, domainsMap] = matchedRules;
  [...anchors].forEach((a) => {
    const matchedDomain = Object.keys(domainsMap).find((domain) => new RegExp(domain).test(a.href));
...
async function decoratePlaceholders(area, config) {
  ...
  const { decoratePlaceholderArea } = await import("./placeholders-CTN6ZPX7.js");
...
async function decorateFooterPromo(doc = document) {
  ...
  const { default: initFooterPromo } = await import("./footer-promo-F7R5MG7M.js");
...
async function loadMartech({
  ...
  const { default: initMartech } = await import("./martech-IAZXAYEY.js");
...
async function checkForPageMods() {
  ...
  const { init } = await import("./personalization-ON6N7TPG.js");
  await init({
    mepParam,
    mepHighlight,
    mepButton,
...
async function loadPostLCP(config) {
  ...
  if (georouting === "on") {
    const { default: loadGeoRouting } = await import("./georoutingv2-X7J4IHZI.js");
  ...
  if (config?.mep) {
    import("./personalization-ON6N7TPG.js").then(({ addMepAnalytics }) => addMepAnalytics(config, header));
  }
}
...
async function loadDeferred(area, blocks, config) {
  ...
  if (config.locale?.ietf === "ja-JP") {
    import("./japanese-word-wrap-77LRLPYO.js").then(({ default: controlJapaneseLineBreaks }) => {
  ...
  if (config.mep?.preview) {
    import("./preview-LL5SXGTV.js").then(({ default: decoratePreviewMode }) => decoratePreviewMode());
  }
  if (config?.dynamicNavKey && config?.env?.name !== "prod") {
    ...
    const { default: loadDNStatus } = await import("./status-I6ZR5NJO.js");
    loadDNStatus();
  }
}
function initSidekick() {
  const initPlugins = async () => {
    const { default: init } = await import("./sidekick-PCCSGDU4.js");
...
function decorateMeta() {
  ...
  window.addEventListener("modal:open", async (e) => {
    ...
    const { findDetails, getModal } = await import("./modal-EFZY44VD.js");
...
async function documentPostSectionLoading(config) {
  ...
  initSidekick();
  const { default: delayed } = await import("./delayed-HNOHT2CA.js");
  delayed([getConfig, getMetadata, loadScript, loadStyle, loadIms]);
  import("./attributes-72BJCYAO.js").then((analytics) => {
...
//# sourceMappingURL=chunk-G4SXHKM5.js.map
