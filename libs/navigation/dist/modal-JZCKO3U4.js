...
var FOCUSABLES = 'a:not(.hide-video), button, input, textarea, select, details, [tabindex]:not([tabindex="-1"]';
var CLOSE_ICON = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
...
var isDelayedModal = false;
var prevHash = "";
var dialogLoadingSet = /* @__PURE__ */ new Set();
...
async function getPathModal(path, dialog) {
  ...
  if (path.includes("/federal/")) {
    const { getFederatedUrl } = await import("./federated-UVUTNMCQ.js");
    href = getFederatedUrl(path);
  }
  const block = createTag("a", { href });
  dialog.append(block);
  const { default: getFragment } = await import("./fragment-WO6WLZWW.js");
...
async function getModal(details, custom) {
  ...
  if (focusablesOnLoad.length && isElementInView(focusablesOnLoad[0])) {
    firstFocusable = focusablesOnLoad[0];
  } else if (titleOnLoad) {
  ...
  if (iframe) {
    if (dialog.classList.contains("commerce-frame")) {
      const { default: enableCommerceFrameFeatures } = await import("./modal.merch-ILIM5VBD.js");
...
function getHashParams(hashStr) {
  ...
  return hashStr.split(":").reduce((params, part) => {
    if (part.startsWith("#")) {
      ...
      if (key === "delay") {
        params.delay = parseInt(val, 10) * 1e3;
      }
    }
    return params;
  }, {});
...
function delayedModal(el) {
  ...
  if (delay === void 0 || !hash) return false;
  isDelayedModal = true;
...
//# sourceMappingURL=modal-JZCKO3U4.js.map
