...
async function getPathModal(path, dialog) {
  ...
  if (path.includes("/federal/")) {
    const { getFederatedUrl } = await import("./federated-DDXH6WXK.js");
    href = getFederatedUrl(path);
  }
  const block = createTag("a", { href });
  dialog.append(block);
  const { default: getFragment } = await import("./fragment-WRBFU54Z.js");
...
async function getModal(details, custom) {
  ...
  if (iframe) {
    if (dialog.classList.contains("commerce-frame") || dialog.classList.contains("dynamic-height")) {
      const { default: enableCommerceFrameFeatures } = await import("./modal.merch-SXGDGSES.js");
...
//# sourceMappingURL=modal-WBOTTJWV.js.map
