...
// ../features/personalization/preview.js
function updatePreviewButton() {
  ...
  document.querySelector(".mep-popup a.con-button").setAttribute("href", simulateHref.href);
}
function getRepo() {
  ...
  manifests?.forEach((manifest) => {
    ...
    variantNames.forEach((variant) => {
      ...
      if (variant === selectedVariantName) {
        checked2.attribute = 'checked="checked"';
        checked2.class = 'class="mep-manifest-selected-variant"';
        manifestParameter.push(`${manifestPath}--${variant}`);
      }
      radio += `<div>
        <input type="radio" name="${manifestPath}" value="${variant}" id="${manifestPath}--${variant}" ${checked2.attribute}>
        <label for="${manifestPath}--${variant}" ${checked2.class}>${variant}</label>
      </div>`;
    });
    const checked = {
      attribute: "",
    ...
    const scheduled = manifest.event ? `<p>Scheduled - ${manifest.disabled ? "inactive" : "active"}</p>
...
export {
  decoratePreviewMode as default
};
//# sourceMappingURL=preview-LL5SXGTV.js.map
