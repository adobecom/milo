...
// ../features/jarvis-chat.js
var chatInitialized = false;
var loadScript;
var loadStyle;
var getMetadata;
var isSilentEvent = (data) => data["event.workflow"] === "init" && data["event.type"] === "request" || data["event.workflow"] === "Chat" && data["event.type"] === "load" && data["event.subtype"] === "window";
var getBaseEvent = (data) => ({
...
var getDataProperties = (data, properties = []) => properties.reduce((str, prop) => {
  let output = str;
  output += `${output && ":"}${data[prop] || ""}`;
  return output;
}, "");
var sendChatIconRenderEvent = (data) => {
...
var sendChatIconClickEvent = (data) => {
  window.digitalData.primaryEvent = {
    eventInfo: {
      eventName: `chat:${getDataProperties(data, ["event.workflow", "event.subcategory", "event.subtype", "content.name", "event.type"])}`,
...
var sendSurveyFeedbackEvent = (data) => {
  window.digitalData.primaryEvent = {
    eventInfo: {
...
var redirectToSupport = () => window.location.assign("https://helpx.adobe.com");
var isChatOpen = () => {
...
var startInitialization = async (config, event, onDemand) => {
  ...
  window.AdobeMessagingExperienceClient.initialize({
    ...
    accessToken: window.adobeIMS?.isSignedInUser() ? `Bearer ${window.adobeIMS.getAccessToken()?.token}` : void 0,
    lazyLoad: true,
    loadedVia: "milo",
    ...
    cookies: {
      mcid: window.alloy ? await window.alloy("getIdentity").then((data) => data?.identity?.ECID).catch(() => void 0) : void 0
    },
    callbacks: {
      initCallback: (data) => {
...
var initJarvisChat = async (config, loadScriptFunction, loadStyleFunction, getMetadataFunction) => {
...
//# sourceMappingURL=jarvis-chat-6NO6BTW2.js.map
