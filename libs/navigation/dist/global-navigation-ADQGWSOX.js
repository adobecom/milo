...
// ../blocks/global-navigation/global-navigation.js
var SIGNIN_CONTEXT = getConfig()?.signInContext;
...
var decorateSignIn = async ({ rawElem, decoratedElem }) => {
...
var decorateProfileTrigger = async ({ avatar }) => {
...
var keyboardNav;
var setupKeyboardNav = async () => {
  keyboardNav = keyboardNav || new Promise(async (resolve) => {
    const { default: KeyboardNavigation } = await import("./keyboard-75NGSGNY.js");
    const instance = new KeyboardNavigation();
    resolve(instance);
  });
};
var getBrandImage = (image, brandImageOnly) => {
...
var convertToPascalCase = (str) => str?.split("-").map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join(" ");
var Gnav = class {
  ...
  setupUniversalNav = () => {
    ...
    this.universalNavComponents = meta?.split(",").map((option) => option.trim()).filter((component) => Object.keys(CONFIG.universalNav.components).includes(component) || component === "signup");
  ...
  ims = async () => window.adobeIMS?.initialized ? this.imsReady() : loadIms().then(() => this.imsReady()).catch((e) => {
  ...
  addChangeEventListeners = () => {
    isDesktop.addEventListener("change", () => {
      if (isDesktop.matches) {
        if (this.elements.mainNav instanceof HTMLElement && this.elements.search instanceof HTMLElement) {
          this.elements.mainNav.after(this.elements.search);
        }
        if (this.elements.topnav instanceof HTMLElement && this.elements.breadcrumbsWrapper instanceof HTMLElement) {
        ...
        if (this.elements.navWrapper instanceof HTMLElement && this.elements.breadcrumbsWrapper instanceof HTMLElement) {
          this.elements.navWrapper.prepend(this.elements.breadcrumbsWrapper);
        }
      }
    });
    const toggleContraction = () => {
      const isOverflowing = isTangentToViewport.matches && this.elements.topnav?.scrollWidth && this.elements.topnav.scrollWidth > document.body.clientWidth;
      ...
      try {
        ...
        if (this.searchPresent()) {
          const [
            ...
            import("./gnav-search-2RRIJAAO.js"),
  ...
  decorateUniversalNav = async () => {
    ...
    const visitorGuid = window.alloy ? await window.alloy("getIdentity").then((data) => data?.identity?.ECID).catch(() => void 0) : void 0;
  ...
  decorateAppPrompt = async ({ getAnchorState } = {}) => {
    ...
    if (state === "off" || !window.adobeIMS?.isSignedInUser() || !isDesktop.matches || hasMobileUA || !entName?.length || !promptPath?.length) return;
    const { base } = getConfig();
    const [
      webappPrompt
    ] = await Promise.all([
      import("./webapp-prompt-HNM7HDME.js"),
  ...
  decorateGenericLogo = ({ selector, classPrefix, includeLabel = true, analyticsValue } = {}) => {
    ...
    const link = blockLinks.find((blockLink) => !imgRegex.test(blockLink.href) && !imgRegex.test(blockLink.textContent));
    if (!link) return "";
    ...
    const getImageEl = () => {
      if (isDarkMode()) {
        ...
        const images = blockLinks.filter((blockLink) => imgRegex.test(blockLink.href) || imgRegex.test(blockLink.textContent));
      ...
      const image = blockLinks.find((blockLink) => imgRegex.test(blockLink.href) || imgRegex.test(blockLink.textContent));
      return getBrandImage(image, isBrandImage);
    };
    const brandImageClass = isBrandImage ? ` ${selectors.brandImageOnly.slice(1)}` : "";
    const imageEl = renderImage ? toFragment`<span class="${classPrefix}-image${brandImageClass}">${getImageEl()}</span>` : "";
  ...
  decorateAside = async () => {
    ...
    const { default: decorate } = await import("./aside-OM57QZCT.js");
  ...
  decorateMainNav = async () => {
    ...
    `;
    const items = [...this.content.querySelectorAll("h2, p:only-child > strong > a, p:only-child > em > a")].filter((item) => CONFIG.features.every((feature) => !item.closest(`.${feature}`)));
  ...
  getMainNavItemType = (item) => {
    ...
    const hasSyncDropdown = itemTopParent instanceof HTMLElement && !isLinkGroup && itemTopParent.childElementCount > 1;
    if (hasSyncDropdown) return "syncDropdownTrigger";
    const hasAsyncDropdown = itemTopParent instanceof HTMLElement && itemTopParent.closest(".large-menu") instanceof HTMLElement;
  ...
  decorateMainNavItem = (item, index) => {
    ...
    const itemHasActiveLink = ["syncDropdownTrigger", "link"].includes(itemType) && getActiveLink(item.closest("div")) instanceof HTMLElement;
  ...
  decorateBreadcrumbs = async () => {
    ...
    const { default: createBreadcrumbs } = await import("./breadcrumbs-AK5PL3GT.js");
...
var getSource = async () => {
  ...
  if (dynamicNavKey) {
    const { default: dynamicNav } = await import("./dynamic-navigation-MNVUFE3B.js");
...
//# sourceMappingURL=global-navigation-ADQGWSOX.js.map
