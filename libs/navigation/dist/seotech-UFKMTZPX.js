...
var SEOTECH_API_URL_PROD = "https://14257-seotech.adobeioruntime.net";
var SEOTECH_API_URL_STAGE = "https://14257-seotech-stage.adobeioruntime.net";
...
async function getVideoObject(url, options) {
  const { env } = options;
...
async function getStructuredData(bucket, id, options) {
...
async function appendScriptTag({ locationUrl, getMetadata, createTag, getConfig }) {
  ...
  if (getMetadata("seotech-video-url")) {
    ...
    promises.push(getVideoObject(getMetadata("seotech-video-url"), { env }).then((videoObject) => append(videoObject, "seotech-video-url")).catch((e) => logError(e.message)));
  }
...
//# sourceMappingURL=seotech-UFKMTZPX.js.map
