...
// ../features/webapp-prompt/webapp-prompt.js
var DISMISSAL_CONFIG = {
  animationCount: 2,
...
var getElemText = (elem) => elem?.textContent?.trim();
var getMetadata = (el) => [...el.childNodes].reduce((acc, row) => {
...
var showTooltip = (element, message = CONFIG.tooltipMessage, time = CONFIG.tooltipDuration) => {
...
var playFocusAnimation = (element, iterationCount = CONFIG.animationCount, animationDuration = CONFIG.animationDuration) => {
...
var modalsActive = () => !!document.querySelector(".dialog-modal");
var waitForClosedModalsThen = (loadPEP) => {
  if (modalsActive()) {
...
var AppPrompt = class _AppPrompt {
  ...
  init = async () => {
    ...
    ({ id: this.anchorId, isOpen: this.isAnchorExpanded } = await this.getAnchorState().catch((e) => {
      lanaLog({
    ...
    this.offset = this.anchor ? this.parent.offsetWidth - (this.anchor.offsetWidth + this.anchor.offsetLeft) : 0;
  ...
  decorate = () => {
    ...
    this.elements.profile = this.profile ? toFragment`<div class="appPrompt-profile">
  ...
  addEventListeners = () => {
    ...
    [this.elements.closeIcon, this.elements.cta].forEach((elem) => elem.addEventListener("click", this.close));
  };
  handleKeyDown = (event) => {
  ...
  initRedirect = () => setTimeout(() => {
    ...
    _AppPrompt.redirectTo(this.options["redirect-url"]);
  }, this.options["loader-duration"]);
  isDismissedPrompt = () => _AppPrompt.getDismissedPrompts().includes(this.id);
  setDismissedPrompt = () => {
    const dismissedPrompts = new Set(_AppPrompt.getDismissedPrompts());
  ...
  static getDismissedPrompts = () => {
    const cookie = document.cookie.split(";").find((item) => item.trim().startsWith("dismissedAppPrompts="))?.split("=")[1];
...
//# sourceMappingURL=webapp-prompt-SDK5JF5J.js.map
