...
var MILO_EVENTS = { DEFERRED: "milo:deferred" };
var LANGSTORE = "langstore";
var PREVIEW = "target-preview";
var PAGE_URL = new URL(window.location.href);
var SLD = PAGE_URL.hostname.includes(".aem.") ? "aem" : "hlx";
var PROMO_PARAM = "promo";
function getEnv(conf) {
  ...
  if (host.includes(`${SLD}.page`) || host.includes(`${SLD}.live`) || host.includes("stage.adobe") || host.includes("corp.adobe") || host.includes("graybox.adobe")) {
...
var [setConfig, updateConfig, getConfig] = /* @__PURE__ */ (() => {
  let config = {};
  return [
    (conf) => {
      ...
      config.useDotHtml = !PAGE_URL.origin.includes(`.${SLD}.`) && (conf.useDotHtml ?? PAGE_URL.pathname.endsWith(".html"));
...
function localizeLink(href, originHostName = window.location.hostname, overrideDomain = false) {
  try {
    ...
    const isLocalizedLink = path.startsWith(`/${LANGSTORE}`) || path.startsWith(`/${PREVIEW}`) || Object.keys(locales).some((loc) => loc !== "" && (path.startsWith(`/${loc}/`) || path.endsWith(`/${loc}`)));
...
var loadScript = (url, type, { mode } = {}) => new Promise((resolve, reject) => {
    ...
    (async () => {
      try {
        ...
        if (config.env.name !== "prod") {
          const { showError } = await import("./fallback-R22QXWJ5.js");
...
function decorateSVG(a) {
  ...
  try {
    ...
    const src = authoredUrl.hostname.includes(".hlx.") || authoredUrl.hostname.includes(".aem.") ? authoredUrl.pathname : authoredUrl;
  ...
  [...images].forEach((img) => {
    ...
    try {
      ...
      if (href.includes(".mp4")) {
        ...
        if (icon) {
          import("./image-video-link-BXT53NYU.js").then((mod) => mod.default(picParent, aTag, icon));
        } else {
          aTag.append(pic);
        }
      }
...
function convertStageLinks({ anchors, config, hostname, href }) {
  ...
  const matchedRules = Object.entries(stageDomainsMap).find(([domain]) => new RegExp(domain).test(href));
  ...
  [...anchors].forEach((a) => {
    ...
    const matchedDomain = Object.keys(domainsMap).find((domain) => new RegExp(domain).test(noLocaleLink));
    if (!matchedDomain) return;
    const convertedLink = noLocaleLink.replace(
      new RegExp(matchedDomain),
      domainsMap[matchedDomain] === "origin" ? `${matchedDomain.includes("https") ? "https://" : ""}${hostname}` : domainsMap[matchedDomain]
    );
...
function decorateHeader() {
  ...
  const dynamicNavActive = getMetadata("dynamic-nav") === "on" && window.sessionStorage.getItem("gnavSource") !== null;
...
async function decorateIcons(area, config) {
  ...
  const { default: loadIcons } = await import("./icons-2SSTIN6G.js");
...
async function decoratePlaceholders(area, config) {
  ...
  placeholderRequest = placeholderRequest || customFetch({ resource: placeholderPath, withCacheRules: true }).catch(() => ({}));
  const { decoratePlaceholderArea } = await import("./placeholders-HRFGGIOR.js");
...
function filterDuplicatedLinkBlocks(blocks) {
  ...
  const uniqueModalKeys = /* @__PURE__ */ new Set();
  const uniqueBlocks = [];
...
function decorateSection(section, idx) {
  ...
  const blockLinks = [...blocks].reduce((blkLinks, block) => {
    ...
    links.filter((link) => block.contains(link)).forEach((link) => {
      if (link.classList.contains("fragment") && MILO_BLOCKS.includes(blockName) && !doNotInline.includes(blockName)) {
...
async function decorateFooterPromo(doc = document) {
  ...
  const { default: initFooterPromo } = await import("./footer-promo-RO3LMJQB.js");
...
var getMepEnablement = (mdKey, paramKey = false) => {
...
async function loadIms() {
  imsLoaded = imsLoaded || new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => reject(new Error("IMS timeout")), imsTimeout || 5e3);
    window.adobeid = {
...
async function loadMartech({
  ...
  if (PAGE_URL.searchParams.get("martech") === "off" || PAGE_URL.searchParams.get("marketingtech") === "off" || getMetadata("martech") === "off") {
    return false;
  }
  window.targetGlobalSettings = { bodyHidingEnabled: false };
  loadIms().catch(() => {
  });
  const { default: initMartech } = await import("./martech-WZTIIXGW.js");
...
async function checkForPageMods() {
  ...
  if (!(pzn || target || promo || mepParam || mepHighlight || mepButton || mepParam === "" || xlg)) return;
  if (target || xlg) {
    loadMartech();
  ...
  const { init } = await import("./personalization-DCXT7K44.js");
  await init({
    mepParam,
    mepHighlight,
    mepButton,
...
async function loadPostLCP(config) {
  ...
  if (sk) import("./sidekick-decorate-DJSQ7GPH.js").then((mod) => {
    mod.default(sk);
  });
  ...
  if (georouting === "on") {
    const { default: loadGeoRouting } = await import("./georoutingv2-L24H7P4L.js");
  ...
  loadTemplate();
  const { default: loadFonts } = await import("./fonts-ELNJCEE3.js");
  loadFonts(config.locale, loadStyle);
  if (config?.mep) {
    import("./personalization-DCXT7K44.js").then(({ addMepAnalytics }) => addMepAnalytics(config, header));
  }
}
...
async function loadDeferred(area, blocks, config) {
  ...
  if (config.links === "on") {
    ...
    import("./links-6DJF7PTH.js").then((mod) => mod.default(path, area));
  }
  if (config.locale?.ietf === "ja-JP") {
    import("./japanese-word-wrap-YO7MCKS2.js").then(({ default: controlJapaneseLineBreaks }) => {
      controlJapaneseLineBreaks(config, area);
    });
  }
  if (getMetadata("pageperf") === "on") {
    import("./logWebVitals-NC52TIG5.js").then((mod) => mod.default(getConfig().mep, {
      delay: getMetadata("pageperf-delay"),
      sampleRate: parseInt(getMetadata("pageperf-rate"), 10)
    }));
  }
  if (config.mep?.preview) {
    import("./preview-Z4GLPTFU.js").then(({ default: decoratePreviewMode }) => decoratePreviewMode());
  }
  if (config?.dynamicNavKey && config?.env?.name !== "prod") {
    ...
    const { default: loadDNStatus } = await import("./status-CKLF3XKF.js");
    loadDNStatus();
  }
}
function initSidekick() {
  const initPlugins = async () => {
    const { default: init } = await import("./sidekick-QZNX6CDT.js");
...
function decorateMeta() {
  ...
  window.addEventListener("modal:open", async (e) => {
    ...
    const { findDetails, getModal } = await import("./modal-JZCKO3U4.js");
...
async function documentPostSectionLoading(config) {
  ...
  if (appendage) {
    import("./title-append-2CWER3AB.js").then((module) => module.default(appendage));
  }
  if (getMetadata("seotech-structured-data") === "on" || getMetadata("seotech-video-url")) {
    import("./seotech-UFKMTZPX.js").then((module) => module.default(
      { locationUrl: window.location.href, getMetadata, createTag, getConfig }
    ));
  }
  const richResults = getMetadata("richresults");
  if (richResults) {
    const { default: addRichResults } = await import("./richresults-GAU5FOJP.js");
    addRichResults(richResults, { createTag, getMetadata });
  }
  loadFooter();
  const { default: loadFavIcon } = await import("./favicon-EZDLHCMZ.js");
  ...
  initSidekick();
  const { default: delayed } = await import("./delayed-CEINYTGH.js");
  delayed([getConfig, getMetadata, loadScript, loadStyle, loadIms]);
  import("./attributes-JBNSX35B.js").then((analytics) => {
...
var preloadBlockResources = (blocks = []) => blocks.map((block) => {
  ...
  return hasStyles && new Promise((resolve) => {
    loadStyle(`${blockPath}.css`, resolve);
  });
}).filter(Boolean);
async function resolveInlineFrags(section) {
  ...
  const { default: loadInlineFrags } = await import("./fragment-WO6WLZWW.js");
...
var utf8ToB64 = (str) => window.btoa(unescape(encodeURIComponent(str)));
var b64ToUtf8 = (str) => decodeURIComponent(escape(window.atob(str)));
...
//# sourceMappingURL=chunk-ZEVYWJU7.js.map
