...
// ../features/georoutingv2/georoutingv2.js
var config;
var createTag;
var getMetadata;
var loadBlock;
var loadStyle;
var sendAnalyticsFunc;
var createTabsContainer = (tabNames) => {
...
var getCookie = (name) => document.cookie.split("; ").find((row) => row.startsWith(`${name}=`))?.split("=")[1];
var getAkamaiCode = () => new Promise((resolve, reject) => {
...
async function loadGeoRouting(conf, createTagFunc, getMetadataFunc, loadBlockFunc, loadStyleFunc) {
  ...
  for (const url of urls) {
    ...
    if (resp.ok) {
      if (url.includes("georouting.json")) {
        ...
        const { default: loadGeoRoutingOld } = await import("./georouting-B5QLGTOR.js");
        loadGeoRoutingOld(config, createTag, getMetadata, json2);
      }
      break;
    }
  }
...
export {
  loadGeoRouting as default,
  getCookie
};
//# sourceMappingURL=georoutingv2-NT2N3D3D.js.map
