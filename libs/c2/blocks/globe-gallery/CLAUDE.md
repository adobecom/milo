...
- **All nine shipped JS files (`globe-gallery.js`, `src/authoring.js`, `src/shaders.js`,
  `src/materials.js`, `src/a11y.js`, `src/modal.js`, `src/math.js`, `src/interaction.js`,
  `src/cursor.js`) are airbnb-clean — keep them that way (`npx eslint` exit 0, no banners).**
  (Texture loading was merged into `src/materials.js`; the arc-phase geometry into `src/math.js`.) The blanket `/* eslint-disable */` is
...
- **`tick()` is a thin orchestrator** — `computeFrame()` builds one per-frame `frame`
  ...
  `snapToSphereSlot` / `requestNavNudge` callbacks, with `sphereRotX/Y` + the sphere-to-card
  ...
  `cardCenterYawPitch` + the frame-counted `easeInOutCubic` nudge tween (both centring paths share it; `KEY_MODAL_FRAMES` vs `KEY_BROWSE_FRAMES`) with the keyboard gallery's `centerCardOnScreen`). Card textures + materials
  both come from `src/materials.js` (`loadCardTextures` / `createCardMaterial`). Rounded corners are an analytic
...
- **`hub-creative-v1/`, `hub-creative-v2/`, `hub-creative-v3/` are read-only
  ...
  `shaders.js`, `materials.js`, `a11y.js`, `modal.js`, `math.js`, `interaction.js`, `cursor.js`)
