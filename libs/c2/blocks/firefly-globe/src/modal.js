...
export default function createGlobeModal({
  ...
  function revealModalChrome() {
    ...
    if (!chromeNodes) {
      chromeNodes = {
        els: [...chromeEl.querySelectorAll('.firefly-globe-modal-info, .firefly-globe-modal-close,'
          + ' .firefly-globe-modal-nav, .firefly-globe-modal-counter')],
      };
    }
    ...
    chromeNodes.els.forEach((el) => {
      ...
      const base = el.classList.contains('firefly-globe-modal-counter') ? 'translateX(-50%)' : '';
  ...
  function announce(text) {
    const el = chromeEl && chromeEl.querySelector('.firefly-globe-modal-announce');
  ...
  function populateModal(i, speak) {
    ...
    const imgEl = targetEl.querySelector('.firefly-globe-modal-image');
    if (imgEl) {
      if (meta.alt) {
  ...
  function ensureModalRenderer() {
    if (modalRenderer || !modalCanvasEl) return;
    const { W, H } = getViewport();
  ...
  const TAP_KEEP_SEL = 'button, a, .firefly-globe-modal-info, .firefly-globe-modal-counter';
  ...
  function setup() {
    if (!modalCanvasEl) modalCanvasEl = q('.firefly-globe-modal-canvas');
    if (modalCanvasEl && modalCanvasEl.parentNode !== document.body) {
      document.body.appendChild(modalCanvasEl);
    }

    if (!modalEl) modalEl = q('.firefly-globe-modal');
    if (!modalEl) return;
    ...
    if (!chromeEl) chromeEl = q('.firefly-globe-modal-chrome');
    chromeNodes = null;
    const evtRoot = chromeEl || modalEl;
    const alreadyWired = listenersWired;
    listenersWired = true;
    if (!alreadyWired) {
      closeBtn = evtRoot.querySelector('.firefly-globe-modal-close');
      ...
      prevBtn = evtRoot.querySelector('.firefly-globe-modal-nav-prev');
      nextBtn = evtRoot.querySelector('.firefly-globe-modal-nav-next');
