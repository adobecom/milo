...
const SPIN_DAA_PAUSE = 'pause_spin--firefly_globe';
const SPIN_DAA_RESUME = 'resume_spin--firefly_globe';
...
export default function createGlobeControls({ q, labels, getVisible, getHintDismissed, rotate }) {
  ...
  function setup() {
    layerEl = q('.firefly-globe-controls');
    if (!layerEl) return;
    // Tab order, not layout: must land AFTER a11y.setup's nodes.
    layerEl.parentNode?.appendChild(layerEl);
    spinBtn = layerEl.querySelector('.firefly-globe-spin-toggle');
    hintEl = layerEl.querySelector('.firefly-globe-hint');
    rotateBtns = [...layerEl.querySelectorAll('.firefly-globe-rotate')];
