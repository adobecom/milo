...
import { defaultState, utf8_to_b64, getUrlConfig } from './shared-utils.js';
...
const Input = ({ label, type = 'text', prop }) => {
    ...
    const value = {
        [type === 'checkbox' ? 'checked' : 'value']: context.state[prop],
    };
...
const updateCollection = (state) => {
    // console.log('ConsonantCardCollection', ConsonantCardCollection)
    // if (!ConsonantCardCollection) return;
...
const reducer = (state, action) => {
    switch (action.type) {
        ...
        case 'MULTI_SELECT_CHANGE':
            ...
            break;
        // case 'MULTI_SELECT_CHANGE':
        //     const values = action.value.map((i) => i.value).join(',');
        //     return { ...state, [action.prop]: values };
        //     break;
        default:
            console.log('DEFAULT');
            return state;
