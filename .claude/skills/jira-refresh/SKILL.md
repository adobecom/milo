...
- **Python**: any `python3` on PATH. On first run `tracker_tool.py` bootstraps a
  dedicated `.venv` under the user cache dir (`~/.cache/jira-refresh`, outside the
  repo) with its one dependency (`beautifulsoup4`) and re-execs itself under it,
  so the invocations below work regardless of which interpreter starts them. No
  global installs, no files left in the working tree, no manual setup.
