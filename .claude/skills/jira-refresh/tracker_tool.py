...
On first run this bootstraps a dedicated `.venv` under the user cache dir
(`$XDG_CACHE_HOME/jira-refresh`, default `~/.cache/jira-refresh`) with its one
dependency (beautifulsoup4) and re-execs under it, so it works under any python3
on PATH without touching global site-packages or leaving files in the repo.
"""
import argparse
import html
import json
...
def _venv_dir():
    """A per-user cache location OUTSIDE the repo, so the venv never shows up as
    untracked files when switching to a branch that lacks this skill (the local
    .gitignore only masks it on branches that ship the skill). Keyed by the
    tool's real path so separate checkouts/worktrees don't share a venv."""
    import hashlib
    cache = os.environ.get("XDG_CACHE_HOME") or os.path.expanduser("~/.cache")
    tag = hashlib.sha1(os.path.realpath(__file__).encode()).hexdigest()[:8]
    return os.path.join(cache, "jira-refresh", f"venv-{tag}")
