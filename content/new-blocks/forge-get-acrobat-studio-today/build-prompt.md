...
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-893c99a6-ms6qg3l9/libs/c2/blocks/forge-get-acrobat-studio-today` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-893c99a6-374` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/ruchikas/forge/figma-893c99?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/lOFnBFhsYyFWPbSdiPa9us/Hub-%E2%80%94-A.com?node-id=10559-113745
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-893c99a6-ms6qg3l9/libs/c2/blocks/forge-get-acrobat-studio-today/forge-get-acrobat-studio-today.js` (`export default
...
5. Phase 8: Forge owns the push — leave the branch `forge/session-893c99a6-374` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `loose-variant` (score: 0.78)
