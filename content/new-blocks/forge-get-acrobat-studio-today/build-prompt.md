...
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-99f82e35-ms7m07gx/libs/c2/blocks/forge-get-acrobat-studio-today` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-99f82e35-424` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/ruchikas/forge/figma-99f82e?milolibs=local` — run Phase 5 in
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-99f82e35-ms7m07gx/libs/c2/blocks/forge-get-acrobat-studio-today/forge-get-acrobat-studio-today.js` (`export default
...
5. Phase 8: Forge owns the push — leave the branch `forge/session-99f82e35-424` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `new-block` (score: 0.50)
