...
- **Target path:** `/Users/akadkly/Documents/GitHub/milo/.forge-worktrees/session-56b67f37-ms75rsnq/libs/c2/blocks/forge-get-acrobat-studio-today` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-56b67f37-91a` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/akadkly/forge/figma-56b67f?milolibs=local` — run Phase 5 in
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/akadkly/Documents/GitHub/milo/.forge-worktrees/session-56b67f37-ms75rsnq/libs/c2/blocks/forge-get-acrobat-studio-today/forge-get-acrobat-studio-today.js` (`export default
...
5. Phase 8: Forge owns the push — leave the branch `forge/session-56b67f37-91a` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `new-block` (score: 0.53)
