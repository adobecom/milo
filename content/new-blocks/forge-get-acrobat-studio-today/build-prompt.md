# build-block-from-figma — task: forge-get-acrobat-studio-today
...
- **Block name:** `forge-get-acrobat-studio-today`
- **Target path:** `/Users/victor/dev/milo-forge-fresh/.forge-worktrees/session-cdad08cb-mserttj8/libs/c2/blocks/forge-get-acrobat-studio-today` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-cdad08cb-985` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/vhargrave/forge/rich-content-hero-cdad08?milolibs=local` — run Phase 5 in
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-get-acrobat-studio-today`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
- the Figma screenshot at `<session>/screenshots/section-0-ref.png` (resolved by Forge's pipeline; Read it if your provisioned task dir has `figma-frame.png`) is the raster target — match its grid/spacing/colour/media fidelity.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-get-acrobat-studio-today`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-get-acrobat-studio-today.css` — link them from the page head).
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-get-acrobat-studio-today-scoped classes via classList.add)
        v
scoped .forge-get-acrobat-studio-today CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/victor/dev/milo-forge-fresh/.forge-worktrees/session-cdad08cb-mserttj8/libs/c2/blocks/forge-get-acrobat-studio-today/forge-get-acrobat-studio-today.js` (`export default
   async function init(el)`) and `forge-get-acrobat-studio-today.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-get-acrobat-studio-today.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-cdad08cb-985` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `below-threshold` (score: 0.61)
