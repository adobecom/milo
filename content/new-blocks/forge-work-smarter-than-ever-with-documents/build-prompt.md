# build-block-from-figma — task: forge-work-smarter-than-ever-with-documents
...
- **Block name:** `forge-work-smarter-than-ever-with-documents`
- **Target path:** `/Users/bradjohn/Documents/forge-new/milo/.forge-worktrees/session-aa00e409-ms9bk8jl/libs/c2/blocks/forge-work-smarter-than-ever-with-documents` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-aa00e409-eb9` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/local-dev/forge/figma-aa00e4?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/fMKQndDRSNu4yTjDYyiQoN/Home-%E2%80%94-A.com?node-id=11612-14386
- **Figma node id:** 11612:14406
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-work-smarter-than-ever-with-documents`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
- the Figma screenshot at `<session>/screenshots/section-3-ref.png` (resolved by Forge's pipeline; Read it if your provisioned task dir has `figma-frame.png`) is the raster target — match its grid/spacing/colour/media fidelity.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-2` /
  `.forge-work-smarter-than-ever-with-documents`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-work-smarter-than-ever-with-documents-scoped classes via classList.add)
        v
scoped .forge-work-smarter-than-ever-with-documents CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/bradjohn/Documents/forge-new/milo/.forge-worktrees/session-aa00e409-ms9bk8jl/libs/c2/blocks/forge-work-smarter-than-ever-with-documents/forge-work-smarter-than-ever-with-documents.js` (`export default
   async function init(el)`) and `forge-work-smarter-than-ever-with-documents.css`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-work-smarter-than-ever-with-documents.test.js`) so forge-block-lint passes.
   **Do NOT touch `libs/utils/utils.js` — Forge registers this block in
   `C2_BLOCKS` itself, deterministically, after read-back.** Your 4 block files
   are the whole job.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-aa00e409-eb9` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `ambiguous` (score: 0.63)
