# build-block-from-figma — task: forge-try-pdf-space-knowledge-hubs-for-free
...
- **Block name:** `forge-try-pdf-space-knowledge-hubs-for-free`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-e0abb9ee-ms3o33dn/libs/c2/blocks/forge-try-pdf-space-knowledge-hubs-for-free` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-6` /
  `.forge-try-pdf-space-knowledge-hubs-for-free`. Use as the starting point for `forge-try-pdf-space-knowledge-hubs-for-free.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-e0abb9ee-ms3o33dn/libs/c2/blocks/forge-try-pdf-space-knowledge-hubs-for-free/forge-try-pdf-space-knowledge-hubs-for-free.js` (`export default
   async function init(el)`) and `forge-try-pdf-space-knowledge-hubs-for-free.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-try-pdf-space-knowledge-hubs-for-free.test.js`) so forge-block-lint passes.
