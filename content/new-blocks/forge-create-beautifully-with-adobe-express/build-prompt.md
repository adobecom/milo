# build-block-from-figma — task: forge-create-beautifully-with-adobe-express
...
- **Block name:** `forge-create-beautifully-with-adobe-express`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-58469d77-mrzb8sex/libs/c2/blocks/forge-create-beautifully-with-adobe-express` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-5` /
  `.forge-create-beautifully-with-adobe-express`. Use as the starting point for `forge-create-beautifully-with-adobe-express.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-58469d77-mrzb8sex/libs/c2/blocks/forge-create-beautifully-with-adobe-express/forge-create-beautifully-with-adobe-express.js` (`export default
   async function init(el)`) and `forge-create-beautifully-with-adobe-express.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-create-beautifully-with-adobe-express.test.js`) so forge-block-lint passes.
