# build-block-from-figma — task: forge-tools-that-work-for-you-3
...
- **Block name:** `forge-tools-that-work-for-you-3`
- **Target path:** `/Users/osahin/repos/milo-logs-deploy-reuse-run/src/forge/shared/data/_repos/milo/.forge-worktrees/session-c5305ca8-ms7li4hk/libs/c2/blocks/forge-tools-that-work-for-you-3` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-c5305ca8-011` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/osahin/forge/figma-c5305c?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/lOFnBFhsYyFWPbSdiPa9us/Hub-A.com?node-id=10559-113821
- **Figma node id:** 10559:113802
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-tools-that-work-for-you-3`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-3` /
  `.forge-tools-that-work-for-you-3`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-tools-that-work-for-you-3-scoped classes via classList.add)
        v
scoped .forge-tools-that-work-for-you-3 CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo-logs-deploy-reuse-run/src/forge/shared/data/_repos/milo/.forge-worktrees/session-c5305ca8-ms7li4hk/libs/c2/blocks/forge-tools-that-work-for-you-3/forge-tools-that-work-for-you-3.js` (`export default
   async function init(el)`) and `forge-tools-that-work-for-you-3.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-tools-that-work-for-you-3.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-c5305ca8-011` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `loose-variant` (score: 0.75)
- Closest existing C2 block: `forge-section-5--seed-70522635`

If the score is close to a tight/loose-variant threshold, consider whether
this should instead become a variant proposal on `forge-section-5--seed-70522635`
