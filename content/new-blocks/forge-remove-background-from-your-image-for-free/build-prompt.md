# build-block-from-figma — task: forge-remove-background-from-your-image-for-free
...
- **Block name:** `forge-remove-background-from-your-image-for-free`
- **Target path:** `/Users/logan/dev/milo-logs-deploy/src/forge/shared/data/_repos/milo/.forge-worktrees/session-1a7efc0e-ms6jmhe2/libs/c2/blocks/forge-remove-background-from-your-image-for-free` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-1a7efc0e-e3a` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/lzartman/forge/figma-1a7efc?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/w2VbLPROC36BO0DA0wBgyY/Doodlebug?node-id=11239-129054
- **Figma node id:** 11239:129055
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-remove-background-from-your-image-for-free`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-remove-background-from-your-image-for-free`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-remove-background-from-your-image-for-free-scoped classes via classList.add)
        v
scoped .forge-remove-background-from-your-image-for-free CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/logan/dev/milo-logs-deploy/src/forge/shared/data/_repos/milo/.forge-worktrees/session-1a7efc0e-ms6jmhe2/libs/c2/blocks/forge-remove-background-from-your-image-for-free/forge-remove-background-from-your-image-for-free.js` (`export default
   async function init(el)`) and `forge-remove-background-from-your-image-for-free.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-remove-background-from-your-image-for-free.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-1a7efc0e-e3a` for
