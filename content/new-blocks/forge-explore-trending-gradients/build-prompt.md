# build-block-from-figma — task: forge-explore-trending-gradients
...
- **Block name:** `forge-explore-trending-gradients`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-3531b320-mrzvpztc/libs/c2/blocks/forge-explore-trending-gradients` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-3531b320-c79` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/bradjohn/forge/extract-gradients-from-images-adobe-colo?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-3` /
  `.forge-explore-trending-gradients`. Use as the starting point for `forge-explore-trending-gradients.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-explore-trending-gradients.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-3531b320-mrzvpztc/libs/c2/blocks/forge-explore-trending-gradients/forge-explore-trending-gradients.js` (`export default
   async function init(el)`) and `forge-explore-trending-gradients.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-explore-trending-gradients.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-3531b320-c79` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `new-block` (score: 0.93)
