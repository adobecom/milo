# build-block-from-figma — task: forge-marquee
...
- **Block name:** `forge-marquee`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-72fd63e6-mrvdsznq/libs/c2/blocks/forge-marquee` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-72fd63e6-b55` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/anonymous/forge/adobe-newsroom-72fd63?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-marquee`. Use as the starting point for `forge-marquee.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-marquee.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-72fd63e6-mrvdsznq/libs/c2/blocks/forge-marquee/forge-marquee.js` (`export default
   async function init(el)`) and `forge-marquee.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-marquee.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-72fd63e6-b55` for
