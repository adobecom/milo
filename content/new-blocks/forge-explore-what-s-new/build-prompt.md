# build-block-from-figma — task: forge-explore-what-s-new
...
- **Block name:** `forge-explore-what-s-new`
- **Target path:** `/Users/bradjohn/Documents/forge-new/milo/.forge-worktrees/session-3dc1f062-ms939yzw/libs/c2/blocks/forge-explore-what-s-new` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-explore-what-s-new`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-explore-what-s-new.css` — link them from the page head).
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-explore-what-s-new-scoped classes via classList.add)
        v
scoped .forge-explore-what-s-new CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/bradjohn/Documents/forge-new/milo/.forge-worktrees/session-3dc1f062-ms939yzw/libs/c2/blocks/forge-explore-what-s-new/forge-explore-what-s-new.js` (`export default
   async function init(el)`) and `forge-explore-what-s-new.css`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-explore-what-s-new.test.js`) so forge-block-lint passes.
...
- R3 matcher decision: `ambiguous` (score: 0.65)
