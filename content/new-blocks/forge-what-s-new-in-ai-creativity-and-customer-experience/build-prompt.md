# build-block-from-figma — task: forge-what-s-new-in-ai-creativity-and-customer-experience
...
- **Block name:** `forge-what-s-new-in-ai-creativity-and-customer-experience`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-22cc8b95-ms7x4v2j/libs/c2/blocks/forge-what-s-new-in-ai-creativity-and-customer-experience` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-22cc8b95-53d` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/akadkly/forge/redesign-22cc8b?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-1` /
  `.forge-what-s-new-in-ai-creativity-and-customer-experience`. Use as the starting point for `forge-what-s-new-in-ai-creativity-and-customer-experience.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-what-s-new-in-ai-creativity-and-customer-experience.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-22cc8b95-ms7x4v2j/libs/c2/blocks/forge-what-s-new-in-ai-creativity-and-customer-experience/forge-what-s-new-in-ai-creativity-and-customer-experience.js` (`export default
   async function init(el)`) and `forge-what-s-new-in-ai-creativity-and-customer-experience.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-what-s-new-in-ai-creativity-and-customer-experience.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-22cc8b95-53d` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `new-block` (score: 0.77)
