# build-block-from-figma — task: forge-introducing-color-mode
...
- **Block name:** `forge-introducing-color-mode`
- **Target path:** `/Users/bradjohn/Documents/forge-new/milo/.forge-worktrees/session-3dc1f062-ms939yzw/libs/c2/blocks/forge-introducing-color-mode` (NEVER libs/blocks/)
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-introducing-color-mode`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-1` /
  `.forge-introducing-color-mode`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-introducing-color-mode-scoped classes via classList.add)
        v
scoped .forge-introducing-color-mode CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/bradjohn/Documents/forge-new/milo/.forge-worktrees/session-3dc1f062-ms939yzw/libs/c2/blocks/forge-introducing-color-mode/forge-introducing-color-mode.js` (`export default
   async function init(el)`) and `forge-introducing-color-mode.css`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-introducing-color-mode.test.js`) so forge-block-lint passes.
