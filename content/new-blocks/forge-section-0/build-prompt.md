# build-block-from-figma — task: forge-section-0
...
- **Block name:** `forge-section-0`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-21ffd24a-ms72ayzw/libs/c2/blocks/forge-section-0` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-21ffd24a-51e` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/osahin-forge/forge/figma-21ffd2?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-section-0`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-section-0.css` — link them from the page head).
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-section-0-scoped classes via classList.add)
        v
scoped .forge-section-0 CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-21ffd24a-ms72ayzw/libs/c2/blocks/forge-section-0/forge-section-0.js` (`export default
   async function init(el)`) and `forge-section-0.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-section-0.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-21ffd24a-51e` for
