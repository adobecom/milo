# build-block-from-figma — task: forge-keep-the-momentum-going
...
- **Block name:** `forge-keep-the-momentum-going`
- **Target path:** `/Users/logan/dev/milo-logs-deploy/src/forge/shared/data/_repos/milo/.forge-worktrees/session-fd400633-ms3tari3/libs/c2/blocks/forge-keep-the-momentum-going` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-fd400633-333` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/osahin/forge/figma-fd4006?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/w2VbLPROC36BO0DA0wBgyY/Doodlebug?node-id=13805-73490&m=dev
- **Figma node id:** 13805:73491
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-keep-the-momentum-going`. Use as the starting point for `forge-keep-the-momentum-going.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/logan/dev/milo-logs-deploy/src/forge/shared/data/_repos/milo/.forge-worktrees/session-fd400633-ms3tari3/libs/c2/blocks/forge-keep-the-momentum-going/forge-keep-the-momentum-going.js` (`export default
   async function init(el)`) and `forge-keep-the-momentum-going.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-keep-the-momentum-going.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-fd400633-333` for
