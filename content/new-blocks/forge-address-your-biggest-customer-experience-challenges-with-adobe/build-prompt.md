# build-block-from-figma — task: forge-address-your-biggest-customer-experience-challenges-with-adobe
...
- **Block name:** `forge-address-your-biggest-customer-experience-challenges-with-adobe`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-559a6f60-ms82hgy7/libs/c2/blocks/forge-address-your-biggest-customer-experience-challenges-with-adobe` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-559a6f60-dc1` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/ruchikas/forge/redesign-559a6f?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-2` /
  `.forge-address-your-biggest-customer-experience-challenges-with-adobe`. Use as the starting point for `forge-address-your-biggest-customer-experience-challenges-with-adobe.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-address-your-biggest-customer-experience-challenges-with-adobe.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-559a6f60-ms82hgy7/libs/c2/blocks/forge-address-your-biggest-customer-experience-challenges-with-adobe/forge-address-your-biggest-customer-experience-challenges-with-adobe.js` (`export default
   async function init(el)`) and `forge-address-your-biggest-customer-experience-challenges-with-adobe.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-address-your-biggest-customer-experience-challenges-with-adobe.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-559a6f60-dc1` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `new-block` (score: 0.81)
