# build-block-from-figma — task: forge-offer-sec9943
...
- **Block name:** `forge-offer-sec9943`
- **Target path:** `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-offer-sec9943` (NEVER libs/blocks/)
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-offer-sec9943`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
- the Figma screenshot at `<session>/screenshots/section-4-ref.png` (resolved by Forge's pipeline; Read it if your provisioned task dir has `figma-frame.png`) is the raster target — match its grid/spacing/colour/media fidelity.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-4` /
  `.forge-offer-sec9943`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-offer-sec9943-scoped classes via classList.add)
        v
scoped .forge-offer-sec9943 CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-offer-sec9943/forge-offer-sec9943.js` (`export default
   async function init(el)`) and `forge-offer-sec9943.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-offer-sec9943.test.js`) so forge-block-lint passes.
