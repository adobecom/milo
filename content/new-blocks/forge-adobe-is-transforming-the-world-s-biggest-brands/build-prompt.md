# build-block-from-figma — task: forge-adobe-is-transforming-the-world-s-biggest-brands
...
- **Block name:** `forge-adobe-is-transforming-the-world-s-biggest-brands`
- **Target path:** `/Users/osahin/repos/milo-logs-deploy-stage-e2e/src/forge/shared/data/_repos/milo/.forge-worktrees/session-99ba7ae9-ms7ez44g/libs/c2/blocks/forge-adobe-is-transforming-the-world-s-biggest-brands` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-99ba7ae9-89e` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/osahin/forge/redesign-99ba7a?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-3` /
  `.forge-adobe-is-transforming-the-world-s-biggest-brands`. Use as the starting point for `forge-adobe-is-transforming-the-world-s-biggest-brands.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-adobe-is-transforming-the-world-s-biggest-brands.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo-logs-deploy-stage-e2e/src/forge/shared/data/_repos/milo/.forge-worktrees/session-99ba7ae9-ms7ez44g/libs/c2/blocks/forge-adobe-is-transforming-the-world-s-biggest-brands/forge-adobe-is-transforming-the-world-s-biggest-brands.js` (`export default
   async function init(el)`) and `forge-adobe-is-transforming-the-world-s-biggest-brands.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-adobe-is-transforming-the-world-s-biggest-brands.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-99ba7ae9-89e` for
