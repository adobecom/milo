# build-block-from-figma — task: forge-create-beautifully
...
- **Block name:** `forge-create-beautifully`
- **Target path:** `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-create-beautifully` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-5` /
  `.forge-create-beautifully`. Use as the starting point for `forge-create-beautifully.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-create-beautifully/forge-create-beautifully.js` (`export default
   async function init(el)`) and `forge-create-beautifully.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-create-beautifully.test.js`) so forge-block-lint passes.
