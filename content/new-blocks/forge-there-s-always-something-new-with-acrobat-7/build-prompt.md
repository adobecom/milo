# build-block-from-figma — task: forge-there-s-always-something-new-with-acrobat-7
...
- **Block name:** `forge-there-s-always-something-new-with-acrobat-7`
- **Target path:** `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-there-s-always-something-new-with-acrobat-7` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-7` /
  `.forge-there-s-always-something-new-with-acrobat-7`. Use as the starting point for `forge-there-s-always-something-new-with-acrobat-7.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-there-s-always-something-new-with-acrobat-7/forge-there-s-always-something-new-with-acrobat-7.js` (`export default
   async function init(el)`) and `forge-there-s-always-something-new-with-acrobat-7.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-there-s-always-something-new-with-acrobat-7.test.js`) so forge-block-lint passes.
