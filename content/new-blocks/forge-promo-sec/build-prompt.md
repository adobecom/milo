# build-block-from-figma — task: forge-promo-sec
...
- **Block name:** `forge-promo-sec`
- **Target path:** `/Users/osahin/repos/milo/.forge-worktrees/session-b8e816dd/libs/c2/blocks/forge-promo-sec` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-b8e816dd-664` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/anonymous/forge/figma-b8e816?milolibs=local` — run Phase 5 in
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-promo-sec`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-2` /
  `.forge-promo-sec`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-promo-sec-scoped classes via classList.add)
        v
scoped .forge-promo-sec CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo/.forge-worktrees/session-b8e816dd/libs/c2/blocks/forge-promo-sec/forge-promo-sec.js` (`export default
   async function init(el)`) and `forge-promo-sec.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-promo-sec.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-b8e816dd-664` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `ambiguous` (score: 0.71)
