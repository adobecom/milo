# build-block-from-figma — task: forge-dark-act
...
- **Block name:** `forge-dark-act`
- **Target path:** `/Users/tek10248/Documents/projects/milo-logs-deploy-result-logs/src/forge/shared/data/_repos/milo/.forge-worktrees/session-c3346e84-ms84jaz0/libs/c2/blocks/forge-dark-act` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-c3346e84-b7c` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/ruchikas/forge/redesign-c3346e?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-4` /
  `.forge-dark-act`. Use as the starting point for `forge-dark-act.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-dark-act.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/tek10248/Documents/projects/milo-logs-deploy-result-logs/src/forge/shared/data/_repos/milo/.forge-worktrees/session-c3346e84-ms84jaz0/libs/c2/blocks/forge-dark-act/forge-dark-act.js` (`export default
   async function init(el)`) and `forge-dark-act.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-dark-act.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-c3346e84-b7c` for
