# build-block-from-figma — task: forge-offer-sec
...
- **Block name:** `forge-offer-sec`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-1eb6381f-mrz68idb/libs/c2/blocks/forge-offer-sec` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-4` /
  `.forge-offer-sec`. Use as the starting point for `forge-offer-sec.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-1eb6381f-mrz68idb/libs/c2/blocks/forge-offer-sec/forge-offer-sec.js` (`export default
   async function init(el)`) and `forge-offer-sec.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-offer-sec.test.js`) so forge-block-lint passes.
