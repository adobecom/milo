# build-block-from-figma — task: forge-act-dark
...
- **Block name:** `forge-act-dark`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-559a6f60-ms82hgy7/libs/c2/blocks/forge-act-dark` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-3` /
  `.forge-act-dark`. Use as the starting point for `forge-act-dark.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-559a6f60-ms82hgy7/libs/c2/blocks/forge-act-dark/forge-act-dark.js` (`export default
   async function init(el)`) and `forge-act-dark.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-act-dark.test.js`) so forge-block-lint passes.
