# build-block-from-figma — task: forge-orchestrate-customer-experiences-with-ai
...
- **Block name:** `forge-orchestrate-customer-experiences-with-ai`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-1e6c8920-ms7wd9bq/libs/c2/blocks/forge-orchestrate-customer-experiences-with-ai` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-1e6c8920-1cf` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/bradjohn/forge/figma-1e6c89?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/fMKQndDRSNu4yTjDYyiQoN/Home-%E2%80%94-A.com?node-id=8450-13352
- **Figma node id:** 9706:13838
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-orchestrate-customer-experiences-with-ai`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-1` /
  `.forge-orchestrate-customer-experiences-with-ai`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-orchestrate-customer-experiences-with-ai-scoped classes via classList.add)
        v
scoped .forge-orchestrate-customer-experiences-with-ai CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-1e6c8920-ms7wd9bq/libs/c2/blocks/forge-orchestrate-customer-experiences-with-ai/forge-orchestrate-customer-experiences-with-ai.js` (`export default
   async function init(el)`) and `forge-orchestrate-customer-experiences-with-ai.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-orchestrate-customer-experiences-with-ai.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-1e6c8920-1cf` for
