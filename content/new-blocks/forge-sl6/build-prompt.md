# build-block-from-figma — task: forge-sl6
...
- **Block name:** `forge-sl6`
- **Target path:** `/data/_repos/milo/.forge-worktrees/session-1e6c8920-ms7wd9bq/libs/c2/blocks/forge-sl6` (NEVER libs/blocks/)
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-sl6`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-5` /
  `.forge-sl6`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-sl6-scoped classes via classList.add)
        v
scoped .forge-sl6 CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/data/_repos/milo/.forge-worktrees/session-1e6c8920-ms7wd9bq/libs/c2/blocks/forge-sl6/forge-sl6.js` (`export default
   async function init(el)`) and `forge-sl6.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-sl6.test.js`) so forge-block-lint passes.
