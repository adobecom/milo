# build-block-from-figma — task: forge-there-s-more-to-acrobat-than-acrobat
...
- **Block name:** `forge-there-s-more-to-acrobat-than-acrobat`
- **Target path:** `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-there-s-more-to-acrobat-than-acrobat` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-526d4ec7-108` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/osahin/forge/figma-526d4e?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/lOFnBFhsYyFWPbSdiPa9us/Hub-%E2%80%94-A.com?node-id=9943-37834
- **Figma node id:** 9943:38008
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-there-s-more-to-acrobat-than-acrobat`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
- the Figma screenshot at `<session>/screenshots/section-2-ref.png` (resolved by Forge's pipeline; Read it if your provisioned task dir has `figma-frame.png`) is the raster target — match its grid/spacing/colour/media fidelity.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-2` /
  `.forge-there-s-more-to-acrobat-than-acrobat`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-there-s-more-to-acrobat-than-acrobat-scoped classes via classList.add)
        v
scoped .forge-there-s-more-to-acrobat-than-acrobat CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo-logs-deploy-ml-judge/src/forge/shared/data/_repos/milo/.forge-worktrees/session-526d4ec7-msoim6my/libs/c2/blocks/forge-there-s-more-to-acrobat-than-acrobat/forge-there-s-more-to-acrobat-than-acrobat.js` (`export default
   async function init(el)`) and `forge-there-s-more-to-acrobat-than-acrobat.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-there-s-more-to-acrobat-than-acrobat.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-526d4ec7-108` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `new-block` (score: 0.57)
