# build-block-from-figma — task: forge-promo-mock
...
- **Block name:** `forge-promo-mock`
- **Target path:** `/Users/osahin/repos/milo/.forge-worktrees/session-e00581d6/libs/c2/blocks/forge-promo-mock` (NEVER libs/blocks/)
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-promo-mock`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-2` /
  `.forge-promo-mock`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-promo-mock-scoped classes via classList.add)
        v
scoped .forge-promo-mock CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/osahin/repos/milo/.forge-worktrees/session-e00581d6/libs/c2/blocks/forge-promo-mock/forge-promo-mock.js` (`export default
   async function init(el)`) and `forge-promo-mock.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-promo-mock.test.js`) so forge-block-lint passes.
...
- R3 matcher decision: `ambiguous` (score: 0.77)
