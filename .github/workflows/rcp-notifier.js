...
const main = async () => {
  ...
  for (const rcp of RCPDates) {
    ...
    const slackText = days => `Reminder RCP starts in ${days} days from ${start.toUTCString()} to ${end.toUTCString()}. Merges to stage will be disabled beginning ${calculateDateOffset(start, stageOffset).toUTCString()}.`
