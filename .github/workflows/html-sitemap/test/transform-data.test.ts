...
test('runTransformData writes sitemap.json from extracted inputs', async () => {
  ...
  const document = JSON.parse(await fs.readFile(path.join(tmpDir, 'business', 'sitemap.json'), 'utf8')) as {
    sections: {
      baseGeoLinks: { heading: string; groups: { subheading: string | null; links: { title: string; url: string }[] }[] }[];
      otherSitemapLinks: { geo: string }[];
      extendedGeoLinks: { geo: string; links: { title: string; path: string }[] }[];
    };
  };
...
test('runTransformData supports www federal GNAV fallback and non-host multi-site eligibility', async () => {
  ...
  const document = JSON.parse(await fs.readFile(path.join(tmpDir, 'www', 'sitemap.json'), 'utf8')) as {
    sections: {
      baseGeoLinks: { heading: string; groups: { links: { url: string }[] }[] }[];
    };
  };
