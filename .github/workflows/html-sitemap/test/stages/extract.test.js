...
test('runExtract writes GNAV, manifest, placeholders, base and extended query indices', async () => {
  ...
  const manifest = JSON.parse(await fs.readFile(path.join(tmpDir, 'business', '_extract', 'gnav', 'manifest.json'), 'utf8'));
  ...
  const brIndex = JSON.parse(await fs.readFile(path.join(tmpDir, 'business', '_extract', 'extended', 'br', 'da-bacom', 'query-index.json'), 'utf8'));
  assert.equal(brIndex.data[0].path, '/br/products');

  const beFrIndex = JSON.parse(await fs.readFile(path.join(tmpDir, 'business', '_extract', 'extended', 'be_fr', 'da-bacom', 'query-index.json'), 'utf8'));
...
test('runExtract warns and continues when GNAV cannot be resolved', async () => {
  ...
  const baseIndex = JSON.parse(await fs.readFile(path.join(tmpDir, 'business', 'fr', '_extract', 'da-bacom', 'query-index.json'), 'utf8'));
