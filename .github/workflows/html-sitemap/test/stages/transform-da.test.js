...
test('runTransformDa writes www html from the federal GNAV-backed document shape', async () => {
  ...
  assert.equal(html, normalizeSnapshotFixture(expectedHtml));
  assert.match(html, /<h2>Creativity &amp; Design<\/h2>/);
  assert.match(html, /<a href="https:\/\/www\.adobe\.com\/creative-cloud\/education">Adobe Express for Education<\/a>/);
  assert.match(html, /<a href="https:\/\/www\.adobe\.com\/products\/photoshop">Photoshop<\/a>/);
